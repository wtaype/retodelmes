...
<section class="sales-table-section">
    ...
    `),Z()}async function Z(a){try{const e=new Date().toISOString().slice(0,7);S=e,s("#monthSelector").val(e),await Promise.all([K(),M(),j()]),ea(),R()}catch(e){console.error("Error inicializando dashboard:",e),d("Error cargando datos del dashboard","error")}}async function K(){try{const a=G("empleadosSmile");a&&(b=a,C());const e=F(A(u,"smiles"),q("participa","==","si"));b=(await I(e)).docs.map(t=>({id:t.id,...t.data()})),O("empleadosSmile",b,300),await k(),C()}catch(a){console.error("Error cargando empleados:",a),s("#workersGrid").html(`
        ...
        `)}}function T(a="",e=!1){let i=[...v];if(i=i.filter(o=>o.fechaTour&&o.fechaTour.startsWith(S)),a&&(i=i.filter(o=>o.vendedor===a)),e){const o=new Date().toISOString().split("T")[0];i=i.filter(P=>P.fechaTour===o)}const t=Math.ceil(i.length/x),n=(f-1)*x,l=i.slice(n,n+x).map(o=>{const y=o.vendedor===h?.displayName?`<button class="btn-view" onclick="verDetalleVenta('${o.id}')" title="Ver detalles"><i class="fas fa-eye"></i></button>
            ...
            <tr>
                ...
                <td><div class="action-buttons">${y}</div></div>
            </tr>
        `}).join("");s("#salesTableBody").html(l||'<tr><td colspan="10" class="empty-cell"><i class="fas fa-inbox"></i> No hay ventas para mostrar</td></tr>'),sa(t)}function aa(a){let e=[];return a.esVentaJulio&&e.push('<span class="venta-especial julio">Julio</span>'),a.esVentaSonia&&e.push('<span class="venta-especial sonia">Sonia</span>'),a.esVentaExterna&&e.push('<span class="venta-especial externa">Externa</span>'),e.length>0?e.join(" "):""}function sa(a){if(a<=1){s("#paginationContainer").html("");return}let e='<div class="pagination">';f>1&&(e+=`<button class="page-btn" onclick="cambiarPagina(${f-1})">
            ...
            `);return}r.sort((p,m)=>m[1].puntos-p[1].puntos);const[l,o]=r[0],[P,y]=a.split("-"),c=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],g={ganador:l,puntosGanados:o.puntos,totalVentas:o.ventas,mes:c[parseInt(y)-1],year:P,mesCompleto:a,fechaRegistro:L()};await V(E(u,"ganadores",e),g),J(g)}catch(a){console.error("Error:",a),s("#lastWinner").html(`
            <div class="winner-header">
                <i class="fas fa-trophy"></i>
                <h3>Ganador del Mes Anterior</h3>
            </div>
...
<div class="points-grid">
   ...
   <div class="tab-content" id="prices-tab">
    ...
    `}s(document).on("click",".btn-save",async a=>{a.preventDefault();try{const e=s(".btn-save").attr("data-edit-id"),i=!!e,t=parseInt(s("#cantidadPax").val())||1,n=parseInt(s("#tipoTour option:selected").data("points"))||0,r=s("#vtJulio").prop("checked"),l=s("#vtSonia").prop("checked"),o=s("#vtExterna").prop("checked"),y=r||l||o?0:n*t,c={tipoTour:s("#tipoTour").val(),registroEn:s("#registroEn").val(),nombreCliente:s("#nombreCliente").val(),numeroHabitacion:s("#numeroHabitacion").val(),tipoDocumento:s("#tipoDocumento").val(),numeroDocumento:s("#numeroDocumento").val(),cantidadPax:t,precioUnitario:parseFloat(s("#precioUnitario").val())||0,metodoPago:s("#metodoPago").val(),importeTotal:parseFloat(s("#importeTotal").val())||0,cobroProveedor:parseFloat(s("#cobroProveedor").val())||0,horaSalida:s("#horaSalida").val(),Operador:s("#Operador").val(),Comentario:s("#Comentario").val(),fechaTour:s("#fechaTour").val(),estadoPago:s("#estadoPago").val(),vendedor:h.displayName,puntos:y,email:h.email,qventa:1,fechaRegistro:L(),esVentaJulio:r,esVentaSonia:l,esVentaExterna:o};if(!c.tipoTour||!c.nombreCliente||!c.horaSalida||!c.fechaTour){d("Por favor completa todos los campos obligatorios","error");return}if(i){c.idVenta=e,await V(E(u,"registrosdb",e),c);const g=`vendedor_${h.displayName}`;O(g,c,450);const p=v.findIndex(m=>m.id===e);p!==-1&&(v[p]={id:e,...c}),d("¡Venta actualizada exitosamente!","success")}else{const p=`venta_${Date.now()}`;c.idVenta=p,await V(E(u,"registrosdb",p),c);const m=`vendedor_${h.displayName}`;O(m,c,450),d("¡Venta registrada exitosamente!","success")}N(),await M(),await k(),C(),R()}catch(e){console.error("Error al guardar/actualizar venta:",e),d("Error al procesar la venta. Inténtalo nuevamente.","error")}});s(document).on("change","#vtJulio, #vtSonia, #vtExterna",function(){D()});s(document).on("change","#tipoTour",function(){const a=s(this).find("option:selected").data("price")||0,e=parseInt(s("#cantidadPax").val())||1;s("#precioUnitario").val(a),s("#importeTotal").val(a*e),D()});s(document).on("input","#cantidadPax, #precioUnitario",function(){const a=parseInt(s("#cantidadPax").val())||1,e=parseFloat(s("#precioUnitario").val())||0;s("#importeTotal").val(e*a),D()});s(document).on("click",".tema",async function(){const a=s(this).data("tema");try{await V(E(u,"configuracion",h.displayName),{tema:a,actualizado:L()},{merge:!0}),O("wiTema",a,72),$('Tema guardado <i class="fa-solid fa-circle-check"></i>')}catch(e){console.error(e)}});
