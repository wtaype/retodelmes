import{D as dt,t as I,F as q,a as m,b as F,q as L,c as P,e as E,l as D,h as K,$ as e,G as ut,N as i,H as Q,y as Z,C as B,z as ft,d as S,B as $,j as k,I as j,i as H,E as pt}from"./widev-CZx_InEv.js";let f=null,w=new Date(new Date().toLocaleString("en-US",{timeZone:"America/Lima"})).toISOString().slice(0,7),b="todos",l=0,T=0,v=[],y=[],u=[],A=7,p=[],N=!1,R=null;dt(K,async t=>{if(!t)return window.location.href="/";R=t;try{const o=I("wiSmileTop");if(o)return W(o),q(m,R);const n=(await F(L(P(m,"smiles"),E("usuario","==",t.displayName)))).docs[0].data();D("wiSmile",n,450),W(n),q(m,R)}catch(o){console.error(o)}});async function W(t){f=t,console.log(`✅ Admin: ${t.nombre}`),e(".app").html(mt()),ut();try{await tt(),await _(),await at(),await z(),await nt(),vt(),C(),x(),i("Bienvenido "+t.nombre,"success")}catch(o){console.error("Error init:",o),i("Error al inicializar","error")}}function mt(){return`
    ...
    `}function J(t,o,a){e("#totalVentas").text(`${t.length} ${t.length===1?"venta":"ventas"}`),e("#totalIngresos").text(`S/ ${o.toFixed(2)}`),e("#totalPuntos").text(`${a} puntos`)}function gt(){const t=e("#pagination"),o=e("#prevPage"),a=e("#nextPage"),n=e("#pageNumbers");if(T<=1){t.hide();return}let s="";const r=8;let d=Math.max(0,l-Math.floor(r/2)),h=Math.min(T-1,d+r-1);h-d<r-1&&(d=Math.max(0,h-r+1));for(let g=d;g<=h;g++)s+=`
        ...
        `;n.html(s),o.prop("disabled",l===0),a.prop("disabled",l>=T-1),t.show()}async function z(){try{console.log("🔄 Cargando tours...");const t=I("toursSmileTop");if(t&&t.length>0){p=t,console.log(`✅ ${p.length} tours desde cache`),M();return}console.log("📡 Cargando desde Firestore...");const o=await F(P(m,"listatours"));if(o.empty){console.log("📭 No hay tours en Firestore"),p=[],M();return}p=o.docs.map(a=>({id:a.id,...a.data()})),D("toursSmileTop",p,300),console.log(`✅ ${p.length} tours cargados y guardados en cache`),M()}catch(t){console.error("❌ Error cargando tours:",t),i("Error al cargar tours","error"),e("#toursContainer").html(`
            <div class="empty-state">
