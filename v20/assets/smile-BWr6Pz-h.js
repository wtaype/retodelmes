import{t as da,$ as a,v as ua,x as ea,M as _,N as l,y as H,z as pa,d as I,a as m,j as W,i as B,l as A,A as ma,C as fa,b as N,c as V,g as va,B as O,D as Y,q as Q,e as X,h as sa}from"./widev-CyPaIruq.js";let C=null;da(sa,async e=>{if(!e)return window.location.href="/";C=e;try{const t=O("wiSmile");if(t)return ta(t);const s=(await N(Q(V(m,"smiles"),X("usuario","==",e.displayName)))).docs[0].data();A("wiSmile",s,450),ta(s)}catch(t){console.error(t)}});a(document).on("click",".bt_salir",async()=>{await ua(sa),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(t=>localStorage.removeItem(t))}});a(document).on("click",".tab-btn",function(){const e=a(this).data("tab");ea(this,"active"),ea("#"+e+"-tab","active")});a(document).on("click",".bt_cargar",()=>{const e=/^(im\d+|ki\d+|remote:im\d+|dirty:im\d+|dirty:ki\d+|toursSmile|notasSmile)$/;Object.keys(localStorage).filter(t=>e.test(t)).forEach(t=>localStorage.removeItem(t)),_("Actualizado"),setTimeout(()=>location.reload(),800)});let D="2025-09",h=1,G=5,b=[],y=[];function ta(e){console.log(e.nombre),_("Bienvenido "+e.nombre+"!"),a(".app").html(`
        <!-- HEADER SUPERIOR -->
...
<p>Creado con<i class='wicon wi-corazon'></i>by<a class='ftx lkme' href='https://wtaype.github.io/' target='_blank'>@wilder.taype</a>2025 - <span class='wty'></span><span class='abw tm11042025' id='101542394703517594'>| Acerca del app | Actualizado</span><span class='wtu'></span></p>
    ...
    `),ba()}async function ia(){try{console.log("🔄 Cargando notas admin...");const e=O("notasSmile");if(e?.length>0){console.log(`✅ ${e.length} notas desde cache`),F(e);return}const t=await N(V(m,"notas"));if(t.empty){console.log("📭 No hay notas"),F([]);return}const o=t.docs.map(s=>({id:s.id,...s.data()}));A("notasSmile",o,600),console.log(`✅ ${o.length} notas cargadas`),F(o)}catch(e){console.error("❌ Error cargar notas:",e),F([])}}function F(e){const t=e.length>0?`
    ...
    `;a(".descripcion_com").html(t)}function ha(e){if(!e)return"Sin fecha";const[t,o,s]=e.split("-");return`${s}/${o}/${t}`}async function ba(e){try{D=H(),a("#monthSelector").val(D),a("#fechaTour").val(H("fecha")),await Promise.all([na(),q(),ra(),ca(),ia()]),Ea(),L(),x()}catch(t){console.error("Error inicializando dashboard:",t),l("Error cargando datos del dashboard","error")}}async function na(){try{const e=O("empleadosSmile");e&&(y=e,$());const t=Q(V(m,"smiles"),X("participa","==","si"));y=(await N(t)).docs.map(s=>({id:s.id,...s.data()})),A("empleadosSmile",y,300),await R(),$()}catch(e){console.error("Error cargando empleados:",e),a("#workersGrid").html(`
            <div class="error-workers">
                <i class="fas fa-exclamation-triangle"></i>
                Error cargando empleados
            </div>
        `)}}async function R(){try{const t=(await N(V(m,"registrosdb"))).docs.filter(o=>{const s=o.data();return s.fechaTour&&s.fechaTour.startsWith(D)});y.forEach(o=>{const s=t.filter(n=>n.data().vendedor===o.usuario);o.totalPuntos=s.reduce((n,r)=>n+(r.data().puntos||0),0),o.totalVentas=s.reduce((n,r)=>n+(r.data().qventa||0),0)}),y.sort((o,s)=>s.totalPuntos-o.totalPuntos)}catch(e){console.error("Error calculando puntos:",e)}}function $(){const e=y.map((t,o)=>{const s=o+1,n=s===1,r=s===2;return`
        ...
        `}).join("");a("#workersGrid").html(e)}async function q(){try{b=(await N(V(m,"registrosdb"))).docs.map(t=>({id:t.id,...t.data()})),b.sort((t,o)=>{const s=new Date(t.fechaTour||"1970-01-01");return new Date(o.fechaTour||"1970-01-01")-s}),E()}catch(e){console.error("Error cargando ventas:",e),a("#salesTableBody").html(`
            <tr><td colspan="9" class="error-cell">
                <i class="fas fa-exclamation-triangle"></i> Error cargando ventas
            </td></tr>
        `)}}function E(e="",t=!1){let o=[...b];if(o=o.filter(i=>i.fechaTour&&i.fechaTour.startsWith(D)),e&&(o=o.filter(i=>i.vendedor===e)),t){const i=new Date().toISOString().split("T")[0];o=o.filter(c=>c.fechaTour===i)}const s=Math.ceil(o.length/G),n=(h-1)*G,S=o.slice(n,n+G).map(i=>{const f=i.vendedor===C?.displayName?`<button class="btn-view" onclick="verDetalleVenta('${i.id}')" title="Ver detalles"><i class="fas fa-eye"></i></button>
        ...
        `}).join("");a("#salesTableBody").html(S||'<tr><td colspan="11" class="empty-cell"><i class="fas fa-inbox"></i> No hay ventas para mostrar</td></tr>'),ya(s)}function ga(e){return{pagado:"PAGADO",cobrado:"PAGADO",cobrar:"DEUDA"}[e]||"DEUDA"}function ya(e){if(e<=1){a("#paginationContainer").html("");return}let t='<div class="pagination">';h>1&&(t+=`<button class="page-btn" onclick="cambiarPagina(${h-1})">
        ...
        </button>`),t+="</div>",a("#paginationContainer").html(t)}async function ra(){try{const e=Ta(D),t=`${e.replace("-","")}`,o=await va(I(m,"ganadores",t));if(o.exists()){oa(o.data());return}const s=await N(V(m,"registrosdb")),n={};s.docs.forEach(w=>{const g=w.data();if(g.fechaTour?.startsWith(e)){const v=g.vendedor;n[v]||(n[v]={puntos:0,ventas:0}),n[v].puntos+=g.puntos||0,n[v].ventas+=g.qventa||0}});const r=Object.entries(n);if(r.length===0){a("#lastWinner").html(`
            ...
            `);return}r.sort((w,g)=>g[1].puntos-w[1].puntos);const[S,i]=r[0],[c,f]=e.split("-"),T=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],d={ganador:S,puntosGanados:i.puntos,totalVentas:i.ventas,mes:T[parseInt(f)-1],year:c,mesCompleto:e,fechaRegistro:W()};await B(I(m,"ganadores",t),d),oa(d)}catch(e){console.error("Error:",e),a("#lastWinner").html(`
            <div class="winner-header">
                <i class="fas fa-trophy"></i>
                <h3>Ganador del Mes Anterior</h3>
            </div>
        ...
        `)}}function oa(e){const t=y.find(o=>o.usuario===e.ganador||o.nombre===e.ganador);a("#lastWinner").html(`
        <div class="winner-header">
            <i class="fas fa-trophy"></i>
            <h3>Ganador del Mes Anterior</h3>
        </div>
    ...
    `)}function L(){const e=b.filter(c=>c.fechaTour&&c.fechaTour.startsWith(D)),t=H("fecha"),o=e.filter(c=>c.fechaTour===t),s=e.reduce((c,f)=>c+(f.qventa||0),0),n=e.reduce((c,f)=>c+(f.puntos||0),0),r=o.reduce((c,f)=>c+(f.qventa||0),0);a("#totalTours").text(s),a("#totalPuntos").text(n),a("#toursHoy").text(r);const i=[Math.min(r/5*100,100),Math.min(s/50*100,100),Math.min(n/2500*100,100),100];a(".summary-stat").each((c,f)=>{const T=i[c]/100*360;a(f).css({"--progress":`${T}deg`,"--width":`${i[c]}%`})})}function Ea(){const e=y.map(t=>`<option value="${t.usuario}">${t.nombre}</option>`).join("");a("#filterEmployee").html(`
        <option value="">Todos los vendedores</option>
        ${e}
    `)}a(document).on("change","#mostrarn",function(){G=parseInt(a(this).val()),h=1,E(a("#filterEmployee").val())});a(document).on("change","#monthSelector",function(){D=a(this).val(),h=1,Promise.all([R(),q()]).then(()=>{$(),E(),L(),ra()})});a(document).on("change","#filterEmployee",function(){h=1,E(a(this).val())});a(document).on("click","#todayFilter",function(){h=1,E(a("#filterEmployee").val(),!0)});window.cambiarPagina=function(e){h=e,E(a("#filterEmployee").val())};window.verDetalleVenta=function(e){const t=b.find(o=>o.id===e);if(!t){l("Venta no encontrada","error");return}la(t,!0),document.querySelector("#formularioVenta").scrollIntoView({behavior:"smooth",block:"start"}),l("Datos cargados para visualización","info")};window.editarVenta=function(e){const t=b.find(o=>o.id===e);if(!t){l("Venta no encontrada","error");return}la(t,!1),a(".btn-save").html('<i class="fas fa-edit"></i> Actualizar Venta').attr("data-edit-id",e),document.querySelector("#formularioVenta").scrollIntoView({behavior:"smooth",block:"start"}),l("Datos cargados para edición","info")};window.eliminarVenta=function(e){const t=b.find(n=>n.id===e);if(!t){l("Venta no encontrada","error");return}!confirm(`¿Estás seguro de eliminar la venta de "${t.nombreCliente}"?
...
Esta acción es IRREVERSIBLE.`)||Sa(e)};function la(e,t=!1){z(),u=p.find(o=>o.tour===e.tipoTour||e.tipoTour.includes(o.tour.split(" ")[1])),u?(a("#tourDisplay .tour-text").text(u.tour),a("#tipoTour").val(u.tour),a(`.tour-row[data-tour*='"nt":${u.nt}']`).addClass("selected")):(a("#tourDisplay .tour-text").text(e.tipoTour||"🔍 Seleccionar tour..."),a("#tipoTour").val(e.tipoTour||"")),a("#registroEn").val(e.registroEn),a("#nombreCliente").val(e.nombreCliente),a("#numeroHabitacion").val(e.numeroHabitacion||""),a("#tipoDocumento").val(e.tipoDocumento||"dni"),a("#numeroDocumento").val(e.numeroDocumento||""),a("#cantidadPax").val(e.cantidadPax||1),a("#precioUnitario").val(e.precioUnitario||0),a("#metodoPago").val(e.metodoPago||""),a("#importeTotal").val(e.importeTotal||0),a("#ganancia").val(e.ganancia||0),k(),a("#horaSalida").val(e.horaSalida),a("#Operador").val(e.Operador),a("#PagoOperador").val(e.PagoOperador||0),a("#Comentario").val(e.Comentario),a("#fechaTour").val(e.fechaTour),a("#estadoPago").val(e.estadoPago||"pagado"),a("#vtJulio").prop("checked",e.esVentaJulio||!1),a("#vtSonia").prop("checked",e.esVentaSonia||!1),a("#vtExterna").prop("checked",e.esVentaExterna||!1),U(),t?(a("#formularioVenta input, #formularioVenta select, .tour-display").prop("disabled",!0),a(".btn-save").prop("disabled",!0).html('<i class="fas fa-eye"></i> Solo Vista'),a("#formularioVenta").addClass("view-only"),a(".btn-clear-view").length===0&&a(".form-actions").prepend(`
            ...
            `))}function U(){const e=parseInt(a("#cantidadPax").val())||1,t=u?u.pts:0,s=a("#vtJulio").prop("checked")||a("#vtSonia").prop("checked")||a("#vtExterna").prop("checked")?0:t*e;a("#vistaPreviaLaPuntos").text(s)}function z(){const e=H("fecha"),t=1;u=null,a("#formularioVenta input, #formularioVenta select").prop("disabled",!1),a(".btn-save").prop("disabled",!1),a("#formularioVenta").removeClass("view-only edit-mode"),a(".btn-save").html('<i class="fas fa-save"></i> Guardar Venta').removeAttr("data-edit-id"),a(".btn-clear-view, .btn-cancel-edit").remove(),a("#formularioVenta")[0].reset(),a("#cantidadPax").val(t),a("#fechaTour").val(e),a("#vistaPreviaLaPuntos").text("0"),a("#tourDisplay .tour-text").text("🔍 Seleccionar tour..."),a(".tour-row").removeClass("selected"),a("#importeTotal").prop("disabled",!0)}async function Sa(e){try{l("Eliminando venta...","info"),await pa(I(m,"registrosdb",e));const t=[];for(let s=0;s<localStorage.length;s++){const n=localStorage.key(s);if(n&&n.startsWith("vendedor_"))try{const r=JSON.parse(localStorage.getItem(n));r&&r.idVenta===e&&t.push(n)}catch{}}t.forEach(s=>localStorage.removeItem(s)),b=b.filter(s=>s.id!==e),await R(),$(),E(),L(),a(".btn-save").attr("data-edit-id")===e&&z(),l("¡Venta eliminada exitosamente!","success")}catch(t){console.error("Error eliminando venta:",t),l("Error al eliminar la venta. Inténtalo nuevamente.","error")}}a(document).on("click",".btn-clear-view",function(){z(),l("Vista limpiada","info")});a(document).on("click",".btn-cancel-edit",function(){z(),l("Edición cancelada","info")});function Ta(e){const[t,o]=e.split("-"),s=new Date(parseInt(t),parseInt(o)-2);return`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`}let p=[];async function ca(){try{console.log("🔄 Cargando tours...");const e=O("toursSmile");if(e?.length>0){p=e.map(s=>({nt:s.num||Math.random(),tour:s.tour,price:s.precio,pts:s.puntos,com:s.comision||5})),console.log(`✅ ${p.length} tours desde cache`),typeof x=="function"&&x();return}const t=await N(Q(V(m,"listatours"),X("activo","==",!0)));if(t.empty){console.log("❌ No hay tours activos"),p=[];return}const o=t.docs.map(s=>({id:s.id,...s.data()}));p=o.map(s=>({nt:s.num||Math.random(),tour:s.tour,price:s.precio,pts:s.puntos,com:s.comision||5})),A("toursSmile",o,300),console.log(`✅ ${p.length} tours cargados desde Firebase`),typeof x=="function"&&x()}catch(e){console.error("❌ Error cargando tours:",e),l("Error al cargar tours","error")}}let u=null;function wa(){return`
    ...
    `}function x(){Pa(p),a("#tourDisplay").off("click"),a("#tourSearch").off("input"),a(document).off("click",".tour-row"),a("#tourDisplay").on("click",function(t){t.stopPropagation();const o=a("#tourDropdown");o.hasClass("active")?(o.removeClass("active"),a("#tourDisplay").removeClass("active")):(o.addClass("active"),a("#tourDisplay").addClass("active"),setTimeout(()=>a("#tourSearch").focus(),50))});let e;a("#tourSearch").on("input",function(){const t=a(this).val().toLowerCase();e&&clearTimeout(e),e=setTimeout(()=>{if(t.length===0)J(p);else if(t.length>=2){const o=p.filter(s=>s.tour.toLowerCase().includes(t)||s.price.toString().includes(t));J(o)}},200)}),a(document).on("click",".tour-row",function(t){t.stopPropagation();const o=a(this).data("index");u=p[o],u&&(a("#tourDisplay .tour-text").text(u.tour),a("#tipoTour").val(u.tour),a("#precioUnitario").val(u.price),a("#tourDropdown").removeClass("active"),a("#tourDisplay").removeClass("active"),a(".tour-row").removeClass("selected"),a(this).addClass("selected"),setTimeout(()=>{Z("#precioUnitario"),K(),U()},50))}),a(document).on("click",function(t){a(t.target).closest(".tour-selector").length||(a("#tourDropdown").removeClass("active"),a("#tourDisplay").removeClass("active"))}),Oa(),Aa()}function J(e){if(!e.length){a("#tourTableBody").html('<tr><td colspan="4" style="text-align:center;color:#666;">No hay tours disponibles</td></tr>');return}const t=document.createDocumentFragment();e.forEach((s,n)=>{const r=document.createElement("tr");r.className="tour-row",r.dataset.index=p.indexOf(s),r.innerHTML=`
            <td class="tour-num">${n+1}</td>
            <td class="tour-name">${s.tour}</td>
            <td class="tour-price">S/ ${s.price}</td>
            <td class="tour-pts">${s.pts} pts</td>
        `,t.appendChild(r)});const o=document.getElementById("tourTableBody");o.innerHTML="",o.appendChild(t)}function Pa(e){J(e)}function k(){const e=a("#estadoPago").val(),t=parseFloat(a("#importeTotal").val())||0,o=parseFloat(a("#PagoOperador").val())||0;let s=0;e==="pagado"?(s=t,a("#PagoOperador").prop("disabled",!0).attr("placeholder","El servicio hemos hecho nosotros").val("0")):(e==="cobrar"||e==="cobrado")&&(s=t-o,a("#PagoOperador").prop("disabled",!1).attr("placeholder","0.00")),a("#ganancia").val(s.toFixed(2)),Z("#ganancia")}a(document).on("change","#estadoPago",function(){k()});a(document).on("input","#importeTotal, #PagoOperador",function(){k()});a(document).on("input","#cantidadPax, #precioUnitario",function(){K(),k()});function Z(e){const t=a(e);t.addClass("field-updated"),setTimeout(()=>{t.removeClass("field-updated")},1e3)}function K(){const e=parseInt(a("#cantidadPax").val())||1,t=parseFloat(a("#precioUnitario").val())||0;a("#importeTotal").val((t*e).toFixed(2)),k(),Z("#importeTotal")}function Ca(){return`
...
<div class="tab-content active" id="points-tab">
    ...
    `}function Oa(){if(!p.length){a("#pointsGrid").html('<p style="text-align:center;color:#666;">No hay tours disponibles</p>');return}const t=[...p].sort((o,s)=>s.pts-o.pts).map(o=>`
        <div class="point-item">
            <span class="service-name">${o.tour}</span>
            <span class="point-value">${o.pts}</span>
        </div>
    `).join("");a("#pointsGrid").html(t)}function Aa(){if(!p.length){a("#pricesGrid").html('<p style="text-align:center;color:#666;">No hay precios disponibles</p>');return}const e=p.map(t=>`
        <div class="price-item">
            <span class="service-name">${t.tour}</span>
            <span class="service-price">S/ ${t.price.toFixed(2)}</span>
        </div>
    `).join("");a("#pricesGrid").html(e)}a(document).on("click",".btn-save",async e=>{if(e.preventDefault(),!a(".btn-save").prop("disabled"))try{if(!u){l("⚠️ Selecciona un tour primero","error"),a("#tourDisplay").focus();return}const t=a(".btn-save"),o=t.html();t.prop("disabled",!0).html('<i class="fas fa-spinner fa-spin"></i> Guardando venta...');const s=a(".btn-save").attr("data-edit-id"),n=!!s,r=parseInt(a("#cantidadPax").val())||1,S=a("#vtJulio").prop("checked"),i=a("#vtSonia").prop("checked"),c=a("#vtExterna").prop("checked"),T=S||i||c?0:u.pts*r,d={tipoTour:u.tour,registroEn:a("#registroEn").val(),nombreCliente:a("#nombreCliente").val(),numeroHabitacion:a("#numeroHabitacion").val(),tipoDocumento:a("#tipoDocumento").val(),numeroDocumento:a("#numeroDocumento").val(),cantidadPax:r,precioUnitario:parseFloat(a("#precioUnitario").val())||0,metodoPago:a("#metodoPago").val(),importeTotal:parseFloat(a("#importeTotal").val())||0,ganancia:parseFloat(a("#ganancia").val())||0,horaSalida:a("#horaSalida").val(),Operador:a("#Operador").val(),PagoOperador:parseFloat(a("#PagoOperador").val())||0,Comentario:a("#Comentario").val(),fechaTour:a("#fechaTour").val(),estadoPago:a("#estadoPago").val(),vendedor:C.displayName,puntos:T,email:C.email,qventa:1,fechaRegistro:W(),esVentaJulio:S,esVentaSonia:i,esVentaExterna:c},w=[[u,"#tourDisplay","tour"],[d.nombreCliente,"#nombreCliente","Cliente"],[d.horaSalida,"#horaSalida","Hora"],[d.fechaTour,"#fechaTour","Fecha"],[d.Operador,"#Operador","Operador"],[d.numeroDocumento,"#numeroDocumento","Documento"],[d.metodoPago,"#metodoPago","y Metodo de pago"]];a(".faltaValor, .okValor").removeClass("faltaValor okValor");const g=w.filter(([v,P,M])=>{const aa=v&&v.toString().trim();return a(P).addClass(aa?"okValor":"faltaValor"),aa?null:M}).map(([,,v])=>v).filter(Boolean);if(g.length){t.prop("disabled",!1).html(o),l(`⚠️ Completa: ${g.join(", ")}`,"error"),a(".faltaValor").first().focus();return}if(n){d.idVenta=s,await B(I(m,"registrosdb",s),d);const v=`vendedor_${C.displayName}`;A(v,d,450);const P=b.findIndex(M=>M.id===s);P!==-1&&(b[P]={id:s,...d}),l("¡Venta actualizada exitosamente!","success")}else{const P=`venta_${Date.now()}`;d.idVenta=P,await B(I(m,"registrosdb",P),d);const M=`vendedor_${C.displayName}`;A(M,d,450),l("¡Venta registrada exitosamente!","success")}a(".faltaValor, .okValor").removeClass("faltaValor okValor"),z(),await q(),await R(),$(),L()}catch(t){console.error("Error al guardar/actualizar venta:",t),l("Error al procesar la venta. Inténtalo nuevamente.","error"),a(".btn-save").prop("disabled",!1).html('<i class="fas fa-save"></i> Guardar Venta')}});a(document).on("change","#vtJulio, #vtSonia, #vtExterna",function(){U()});a(document).on("input change","#formularioVenta input, #formularioVenta select",function(){const e=a(this).val()?.toString().trim();a(this).removeClass("faltaValor").addClass(e?"okValor":"faltaValor")});a(document).on("click",".tour-row",function(){a("#tourDisplay").removeClass("faltaValor").addClass("okValor")});a(document).on("change","#tipoTour",function(){const e=a(this).find("option:selected").data("price")||0,t=parseInt(a("#cantidadPax").val())||1;a("#precioUnitario").val(e),a("#importeTotal").val(e*t),U()});a(document).on("input","#cantidadPax, #precioUnitario",function(){K(),U(),k()});a(document).on("click",".tema",async function(){const e=a(this).data("tema");try{await B(I(m,"configuracion",C.displayName),{tema:e,actualizado:W()},{merge:!0}),A("wiTema",e,72),_('Tema guardado <i class="fa-solid fa-circle-check"></i>')}catch(t){console.error(t)}});async function Da(){try{const e=a(".wifresh"),t=e.html();e.html('<i class="fas fa-spinner fa-spin"></i>').prop("disabled",!0),l("🔄 Verificando actualizaciones...","info");const o=await Na();if(!o.hayActualizaciones){e.html(t).prop("disabled",!1),l("✅ Todo está actualizado","success");return}l(`🔄 Aplicando ${o.total} actualizaciones...`,"info");const s=[];o.empleados&&s.push(Va()),o.tours&&s.push(xa()),o.ventas&&s.push(Ia()),o.notas&&s.push($a()),await Promise.all(s),await Promise.all([R(),L()]),$(),E(),x(),e.html(t).prop("disabled",!1),l(`✅ ${o.total} actualizaciones aplicadas correctamente`,"success")}catch(e){console.error("❌ Error en wifresh:",e),a(".wifresh").html('<i class="fa-solid fa-rotate-right"></i>').prop("disabled",!1),l("❌ Error en actualización. Inténtalo nuevamente.","error")}}async function Na(){const e={empleados:!1,tours:!1,ventas:!1,notas:!1,total:0,hayActualizaciones:!1};try{(!O("empleadosSmile")||j("empleadosSmile",300))&&(e.empleados=!0,e.total++),(!O("toursSmile")||j("toursSmile",300))&&(e.tours=!0,e.total++),e.ventas=!0,e.total++,(!O("notasSmile")||j("notasSmile",600))&&(e.notas=!0,e.total++),e.hayActualizaciones=e.total>0}catch(t){console.error("Error detectando cambios:",t)}return e}function j(e,t){try{const o=localStorage.getItem(e+"_timestamp");if(!o)return!0;const s=parseInt(o);return(Date.now()-s)/1e3>t}catch{return!0}}async function Va(){Y("empleadosSmile"),await na()}async function xa(){Y("toursSmile"),await ca()}async function Ia(){await q()}async function $a(){Y("notasSmile"),await ia()}a(document).on("click",".wifresh",function(e){e.preventDefault(),Da()});
