import{$ as i}from"./vendor-PbmUQHyn.js";import{db as y}from"./firebase-BK99ufhq.js";import{i as M,c as S}from"./firebase-BM1KOhEp.js";import{A,u as F,k as q,S as x,e as C,l as P,w as R}from"./index-DIaJmuXG.js";import{getMesActual as D,obtenerRankingMes as T}from"./zsmile-CGfODium.js";let p=D();const Y=()=>`
    <div class="smw_dash">
  ...
  `,j=async()=>{const s=A.user;if(!s)return setTimeout(()=>F.navigate("/login"),100);const a=q(s.nombre||s.usuario||""),e=`${(s.nombre||"?")[0]}${(s.apellidos||"")[0]||""}`.toUpperCase();i("#smwAvatar").text(e),i("#smwSaludo").html(`${x()} <strong>${a}</strong>`),s.descripcion?i("#smwRole").html(`<i class="fas fa-user-tag"></i> ${s.descripcion}`):i("#smwRole").html('<i class="fas fa-car-side"></i> Colaborador — Reto del Mes'),i("#smwMonthSelector").val(p),b(s.usuario,p),i(document).off("change.smile_dash").on("change.smile_dash","#smwMonthSelector",function(){p=i(this).val(),b(s.usuario,p)}),i(".wi_fadeUp").addClass("visible wi_visible"),console.log(`🏜️ ${C} Smile Dashboard cargado`),window.__WIREADY__=!0},J=()=>{i(document).off("change.smile_dash")};async function b(s,a){try{i("#kpiTours").html('<span class="smw_sk_kpi"></span>'),i("#kpiPuntos").html('<span class="smw_sk_kpi"></span>'),i("#kpiPos").html('<span class="smw_sk_kpi"></span>');const e=`kpiSmile_${s}_${a}`,n=P(e);if(n)return f(n);const[g,d]=a.split("-").map(Number),r=await M(S(y,"registrosdb"));let t=0,o=0;r.docs.forEach(u=>{const l=u.data();if(l.vendedor!==s)return;const c=l.fechaTour;let v,w;if(typeof c=="string")[v,w]=c.split("-").map(Number);else if(c?.toDate){const h=c.toDate();v=h.getFullYear(),w=h.getMonth()+1}else return;v===g&&w===d&&(t+=parseInt(l.qventa)||1,o+=parseInt(l.puntos)||0)});const _=(await T(a)).findIndex(u=>u.usuario===s),$=_===-1?"—":`#${_+1}`,k={tours:t,puntos:o,posicion:$};R(e,k,5),f(k)}catch(e){console.warn("KPI error:",e),f({tours:"?",puntos:"?",posicion:"?"})}}function f({tours:s,puntos:a,posicion:e}){i("#kpiTours").text(s),i("#kpiPuntos").text(a),i("#kpiPos").text(e)}function E(){const s=new Date,a=s.getFullYear(),e=s.getMonth(),n=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];return i.map(new Array(7),(g,d)=>{const r=d-3,t=e+r,o=a+Math.floor(t/12),m=(t%12+12)%12;return`<option value="${`${o}-${String(m+1).padStart(2,"0")}`}"${r===0?" selected":""}>${n[m]} ${o}</option>`}).join("")}export{J as cleanup,j as init,Y as render};
