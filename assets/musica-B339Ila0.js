import{$ as a}from"./vendor-PbmUQHyn.js";import{db as f}from"./firebase-QWwQUjRS.js";import{z as N,e as b,t as g,d as L,u as M,c as F,o as j,q as B,p as T}from"./firebase-DzkkQdMV.js";import{j as h,w as G,u as E,b as o,r as C,E as R,d as H}from"./index-BQyVSIlW.js";const p="wimusica",v="wiMusica";let c=[],r=null,q=null;const n=new Audio,e={actual:null,playing:!1,repeat:!1,editando:null},x="/retodelmes/poster.webp",A=t=>isNaN(t)?"0:00":`${~~(t/60)}:${String(~~(t%60)).padStart(2,"0")}`,J=()=>{const t=h("wiSmile");return`
<div class="mwb">
  <div class="mus_layout ${t?.email?"autenticado":""}">
    ...
    </div>`}).join(""))},O=t=>{a("#musActual").text(t.nombre),a("#musArtista").text(t.cantante),a("#musCover").attr("src",t.portada||x),a("#musFav").toggleClass("on",!!t.favorito),a(".mpi").removeClass("on").filter(`[data-id="${t.id}"]`).addClass("on")},k=t=>{t&&(e.actual=t,O(t),n.src=t.url,E("musActual",t.id,168),a(".mpi_ico i").attr("class","fas fa-play"))},_=()=>{if(!e.actual){if(!c.length)return o("Sin canciones","warning");k(c[0])}n.play().then(()=>{e.playing=!0,a("#musPlay i").attr("class","fas fa-pause"),a(".mpi.on .mpi_ico i").attr("class","fas fa-pause")}).catch(()=>o("Error al reproducir","error"))},D=()=>{n.pause(),e.playing=!1,a("#musPlay i").attr("class","fas fa-play"),a(".mpi_ico i").attr("class","fas fa-play")},z=t=>{if(!c.length)return;const u=c.findIndex(l=>l.id===e.actual?.id);k(c[u+t]||c[t>0?0:c.length-1]),_()},S=()=>{a("#fmMusica")[0]?.reset(),a("#fiFav").prop("checked",!1),a("#fiPub").prop("checked",!0),e.editando=null,a("#fmTitle").html('<i class="fas fa-plus-circle"></i> Nueva Canción'),a("#fmSave span").text("Guardar"),a("#fmSave i").attr("class","fas fa-save"),a("#fmDelete, #fmNew, #fgId").hide()},K=t=>{const u=s=>s.publico!==!1,l=(s,i)=>i.favorito-s.favorito||(i.creado?.seconds||0)-(s.creado?.seconds||0);return r?.email?[...t.filter(s=>s.email===r.email),...t.filter(s=>s.email!==r.email&&u(s))].sort(l):t.filter(u).sort(l)},Q=()=>{r=h("wiSmile"),G();const t=h(v);if(t?.length){c=t,$();const l=c.find(s=>s.id===h("musActual"));l&&k(l)}h("musRep")&&(e.repeat=!0,a("#musRep").addClass("on")),a(n).on({"timeupdate.m":()=>{const{currentTime:l,duration:s}=n;isNaN(s)||(a("#musCur").text(A(l)),a("#musDur").text(A(s)),a(".mus_fill").css("width",`${l/s*100}%`))},"ended.m":()=>e.repeat?(n.currentTime=0,_()):z(1),"loadedmetadata.m":()=>a("#musDur").text(A(n.duration))}),a(document).off(".musica").on("click.musica","#musPlay, #musPlay *",()=>e.playing?D():_()).on("click.musica","#musPrev, #musPrev *",()=>z(-1)).on("click.musica","#musNext, #musNext *",()=>z(1)).on("click.musica","#musRep,  #musRep *",()=>{e.repeat=!e.repeat,a("#musRep").toggleClass("on",e.repeat),E("musRep",e.repeat,168)}).on("click.musica","#musFav,  #musFav *",async function(){if(!e.actual||r?.email!==e.actual.email)return o("Sin permisos","warning");const l=!e.actual.favorito;a("#musFav").toggleClass("on");try{await N(b(f,p,e.actual.id),{favorito:l,actualizado:g()}),e.actual.favorito=l,C(v)}catch{a("#musFav").toggleClass("on"),o("Error","error")}}).on("click.musica",".mus_bar",function(l){n.duration&&(n.currentTime=l.offsetX/a(this).width()*n.duration)}).on("click.musica",".mpi",function(l){if(a(l.target).closest(".mpi_acts").length)return;const s=c.find(i=>i.id===a(this).data("id"));s&&(e.actual?.id===s.id?e.playing?D():_():(k(s),_()),r?.email===s.email?(e.editando=s.id,a(".fi_nom").val(s.nombre),a(".fi_art").val(s.cantante),a(".fi_url").val(s.url),a(".fi_por").val(s.portada||""),a(".fi_tag").val(s.tag||"alabanza"),a(".fi_id").val(s.id),a("#fiFav").prop("checked",!!s.favorito),a("#fiPub").prop("checked",s.publico!==!1),a("#fmTitle").html('<i class="fas fa-pen"></i> Editar Canción'),a("#fmSave span").text("Actualizar"),a("#fmSave i").attr("class","fas fa-pen"),a("#fmDelete, #fmNew, #fgId").show()):S())}).on("input.musica","#musSrc",$).on("click.musica",".cat",function(){a(this).hasClass("sug_cat")||(a(".cat").not(".sug_cat").removeClass("active"),a(this).addClass("active"),$())}).on("click.musica",".sug_cat",function(){a(".fi_tag").val(a(this).text()),a(".fi_tag").focus()}).on("click.musica",".fav_t, .fav_t *",async function(l){l.stopPropagation();const s=a(this).closest(".fav_t").data("id"),i=c.find(P=>P.id===s);if(!i||r?.email!==i.email)return o("Sin permisos","warning");const d=!i.favorito,m=a(`.fav_t[data-id="${s}"]`),y=m.closest(".mpi");m.toggleClass("on"),y.toggleClass("fav");try{await N(b(f,p,s),{favorito:d,actualizado:g()}),i.favorito=d,C(v)}catch{m.toggleClass("on"),y.toggleClass("fav"),o("Error","error")}}).on("click.musica","#fmDelete",async function(l){if(!e.editando)return;const s=c.find(i=>i.id===e.editando);if(!s||r?.email!==s.email)return o("Sin permisos","warning");if(confirm(`¿Eliminar "${s.nombre}"?`))try{await L(b(f,p,s.id)),a(`.mpi[data-id="${s.id}"]`).fadeOut(250,function(){a(this).remove()}),e.actual?.id===s.id&&(D(),e.actual=null,a("#musActual").text("Selecciona una canción"),a("#musCover").attr("src",x)),C(v),o("Eliminada","success"),S()}catch{o("Error","error")}}).on("submit.musica","#fmMusica",async function(l){l.preventDefault();const s=a(".fi_nom").val().trim(),i=a(".fi_art").val().trim(),d=a(".fi_url").val().trim(),m=a(".fi_por").val().trim(),y=a(".fi_tag").val(),P=a("#fiFav").is(":checked"),I=a("#fiPub").is(":checked");if(!s||!i||!d||!m)return o("Completa todos los campos","warning");if(!d.toLowerCase().includes(".mp3"))return o("La pista debe ser .mp3","warning");try{R(a("#fmSave")[0],!0);const w={nombre:s,cantante:i,url:d,portada:m,tag:y,favorito:P,publico:I,email:r.email,usuario:r.usuario||r.email.split("@")[0]};e.editando?(await N(b(f,p,e.editando),{...w,actualizado:g()}),o("✅ Actualizada","success")):(await M(b(F(f,p),`musica_${Date.now()}`),{...w,creado:g(),actualizado:g()}),o("✅ Agregada","success")),C(v),S()}catch(w){console.error(w),o("Error","error")}finally{R(a("#fmSave")[0],!1)}}).on("click.musica","#fmNew",S);const u=()=>{q=j(B(F(f,p),T("favorito","desc"),T("creado","desc")),l=>{const s=l.docs.map(i=>({id:i.id,...i.data()}));c=K(s),E(v,c,168),$()},()=>o("Desde caché","info",2e3))};t?.length?setTimeout(u,800):u(),console.log(`✅ Música — ${H}`)},V=()=>{q?.(),n.pause(),n.src="",a(n).off(".m"),a(document).off(".musica")};export{V as cleanup,Q as init,J as render};
