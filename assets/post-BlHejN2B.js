import{v as D,w as f,D as I,d as y,b as M,j as h,G as e,y as E,l as R,I as C,_ as m}from"./index-BQyVSIlW.js";import{a as T}from"./wiad-DWbEudQX.js";import{$ as a}from"./vendor-PbmUQHyn.js";import{i as z,b as L,g,l as V,m as j,k as O,o as w,t as q,a as B,e as N,f as P}from"./devblog-CFkKKv_r.js";import"./firebase-QWwQUjRS.js";import"./firebase-DzkkQdMV.js";const _=s=>`<a href="/${s.slug||s.id}" class="po_rel_card" ${e(s.resumen||s.titulo)}><div class="po_rel_img"><img src="${s.imagen}" alt="${s.imagenAlt||s.titulo}" loading="lazy"/></div><div class="po_rel_info"><span class="po_rel_cat"><i class="fas fa-paw"></i> ${s.categoria}</span><strong>${s.titulo}</strong><span class="po_rel_meta"><i class="fas fa-calendar"></i> ${w(s.actualizado||s.creado)} · <i class="fas fa-eye"></i> ${s.vistas||0} · <i class="fas fa-heart" style="color:#fe0149"></i> ${s.likes||0}</span></div></a>`,b=(s,i,t="po_fade po_visible")=>s||i?`
    ...
    </div>`:"",k=(s,i,t,o,r)=>{const v=E.user?.usuario,l=r?"po_fade po_visible":"po_fade",c=i!==null&&t!==null,d=i||[],n=t||[],x=d[0]||n[0],S=d[1]||n[1],A=c?b(x,S,l):'<div id="po_nav_container"></div>';return`
      ...
      <div class="po_col_main"><div class="po_content">
        ...
        <header class="po_header ${l}" style="--d:.1s">
          ...
          <div class="po_meta">
            ...
            <span ${e("Fecha")}><i class="fas fa-calendar"></i> ${w(s.creado,!0)}</span>
            ...
            ${r?`<span class="po_cache_badge" ${e("Cache ⚡")}><i class="fas fa-bolt"></i> Local</span>`:""}
          </div>
        </header>
        ...
        <div class="po_share ${l}" style="--d:.45s"><span><i class="fas fa-share-nodes"></i> Comparte</span>
          <div class="po_share_btns">
            <button class="po_like_btn po_like_sync ${localStorage.getItem("wi_like_"+o)?"active":""}" data-slug="${o}" ${e("Me encanta")}><i class="fas fa-heart"></i> <span class="po_likes_count_text">${s.likes||0}</span></button>
            ${q(s.titulo)}<button class="po_share_btn po_copy" style="--sc:var(--mco)" ${e("Copiar")}><i class="fas fa-link"></i></button>
          </div>
        </div>
      </div>
    ...
    <aside class="po_sidebar">
      <div class="po_side_card ${l}" style="--d:.15s">
        ...
        <div class="po_autor_box"><div class="po_autor_av"><img src="/retodelmes/smile.avif" alt="${s.autor}"/></div><div class="po_autor_info"><strong>${s.autor}</strong><span>${y} <i class="fas ${R}"></i></span></div></div>
      ...
      <div id="po_ultimas_container">${c?n.length?`<div class="po_side_card ${l}" style="--d:.2s"><div class="po_side_title"><i class="fas fa-clock"></i> Últimas historias</div><div class="po_relacionados" data-showi="100">${n.map(_).join("")}</div></div>`:"":'<div class="po_side_card po_fade po_visible" style="--d:.2s"><div class="po_side_title"><i class="fas fa-clock"></i> Últimas historias</div><div class="po_sk_side shimmer" style="height:120px"></div></div>'}</div>
      ...
      <div class="po_ad_sticky ${l}" style="--d:.3s; position: sticky; top: 8vh; margin-top: 2vh; text-align:center; min-height:600px;">
         ...
         <div class="wi_ad_fallback">${T}</div>
      </div>
    </aside>
  </div></div>`};let p=!1;const W=s=>{const i=s&&h(`wi_post_${s}`);if(i)return p=!0,k(i,h(`wi_mas_${i.categoria}`)||[],h("wi_sidebar_posts")||[],s,!0);p=!1;const t=s&&O(s);return t?`
    <div class="po_wrap"><div class="po_layout"><div class="po_col_main"><div class="po_content">
      ...
      <div class="po_contenido po_fade" style="--d:.1s; min-height:98vh;"><div class="po_sk_body">${'<div class="po_sk_p shimmer"></div>'.repeat(6)}</div></div>
    </div></div><aside class="po_sidebar" style="min-height:98vh;">${'<div class="po_sk_side shimmer"></div>'.repeat(3)}</aside></div></div>`:`<div class="po_wrap"><div class="po_layout"><div class="po_col_main"><div class="po_content"><div class="po_sk_img shimmer"></div><div class="po_sk_body" style="min-height:98vh;"><div class="po_sk_cat shimmer"></div><div class="po_sk_tit shimmer"></div><div class="po_sk_meta shimmer"></div>${'<div class="po_sk_p shimmer"></div>'.repeat(5)}</div></div></div><aside class="po_sidebar" style="min-height:98vh;">${'<div class="po_sk_side shimmer"></div>'.repeat(3)}</aside></div></div>`},U=async(s,i=!1)=>{if(s){p&&!i&&(g("po_fade"),f(),u(),$());try{const t=await z(s,i);if(!t?.data?.activo)return a("#wimain").html('<div class="po_err dpvc"><i class="fas fa-paw"></i><h2>Historia no encontrada</h2><p>No existe o no está disponible 🐾</p><a href="/blog" class="po_back_btn"><i class="fas fa-arrow-left"></i> Ver historias</a></div>');const{data:o,fromCache:r}=t;!r&&!i&&L(s),D({title:o.titulo,desc:o.resumen,keywords:o.keywords,img:o.imagenTop||o.imagen,path:`/${s}`,type:"Article",datePublished:o.creado}),!p||i?(a("#wimain").html(k(o,null,null,s,r)),g("po_fade"),f(),u(),$()):r||a(".po_cache_badge").remove();const v=(c,d)=>{a("#po_nav_container").html(b(c[0]||d[0],c[1]||d[1])),a("#po_ultimas_container").html(d.length?`<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-clock"></i> Últimas historias</div><div class="po_relacionados" data-showi="100">${d.map(_).join("")}</div></div>`:""),a("#po_rels_container").html(c.length?`<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-heart"></i> Más historias de ${o.categoria}</div><div class="po_relacionados" data-showi="100">${c.map(_).join("")}</div></div>`:""),f()};Promise.all([V(s,o.categoria,i),j(s,i)]).then(([c,d])=>{v(c||[],d||[])});const l=function(){this.page.url=`https://wiihope.com/${s}`,this.page.identifier=s,this.page.title=o.titulo};window.DISQUS?window.DISQUS.reset({reload:!0,config:l}):(window.disqus_config=l,I({disqus:[async()=>a("body").append(a("<script>",{src:"https://superwii.disqus.com/embed.js","data-timestamp":+new Date}))]})),console.log(`🐾 ${y} Post OK`),window.__WIREADY__=!0}catch(t){console.error("[post]",t),M("Error al cargar","error")}}};function u(){if(a(".po_toc_box").length)return;const s=a(".po_contenido").find("h2,h3");if(!s.length)return;let i='<div class="po_toc_box po_fade" style="--d:.15s"><div class="po_toc_title"><i class="fas fa-list"></i> En este artículo</div><ul class="po_toc">';s.each((t,o)=>{const r="po_h_"+t;a(o).attr("id",r).css("scroll-margin-top","7vh"),i+=`<li style="margin-left:${o.tagName.toLowerCase()==="h3"?"1.5vh":"0"}"><a href="#${r}" class="po_toc_link">${a(o).text()}</a></li>`}),a(i+"</ul></div>").insertBefore(".po_contenido")}function $(){a(window).off("scroll.post_prog").on("scroll.post_prog",()=>a("#po_progress").css("width",a(window).scrollTop()/Math.max(a(document).height()-a(window).height(),1)*100+"%"))}a(document).on("click.post",".po_copy,.po_copy2",()=>C(location.href,".po_copy","¡Enlace copiado! 🔗")).on("click.post",".po_rel_card",function(s){s.preventDefault(),m(()=>import("./index-BQyVSIlW.js").then(i=>i.t),[]).then(i=>i.rutas.navigate(a(this).attr("href")))}).on("click.post",".po_like_sync",function(){const s=a(this).data("slug");localStorage.getItem("wi_like_"+s)||(localStorage.setItem("wi_like_"+s,"1"),a(".po_like_sync").addClass("active"),a(".po_likes_count_text").text((parseInt(a(".po_likes_count_text").first().text())||0)+1),B(s))}).on("click.post",".po_yt_btn",async function(){const s=a(this).data("yt");a("#wi_yt_modal").length||(a("body").append(`
        ...
        </div>`),a("#wi_yt_modal").on("click",function(t){t.target===this&&a(".wi_yt_close").trigger("click")})),a("#wi_yt_player").html(`<iframe width="100%" height="100%" src="https://www.youtube.com/embed/${s}?autoplay=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`);const{abrirModal:i}=await m(async()=>{const{abrirModal:t}=await import("./index-BQyVSIlW.js").then(o=>o.J);return{abrirModal:t}},[]);i("wi_yt_modal")}).on("click.post",".wi_yt_close",async function(){const{cerrarModal:s}=await m(async()=>{const{cerrarModal:i}=await import("./index-BQyVSIlW.js").then(t=>t.J);return{cerrarModal:i}},[]);s("wi_yt_modal"),setTimeout(()=>a("#wi_yt_player").html(""),300)}).on("click.post","#po_refresh",async function(){const s=a(this);s.html('<i class="fas fa-spinner fa-spin"></i>').prop("disabled",!0),N(s.data("slug")),P(s.data("cat")),await U(s.data("slug"),!0)});const X=()=>a(window).off("scroll.post_prog");export{X as cleanup,U as init,W as render};
