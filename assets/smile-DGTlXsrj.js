...
<section class="sales-table-section">
    ...
    `;e(".descripcion_com").html(t)}function ia(){const a=new Date,t=a.getFullYear(),o=String(a.getMonth()+1).padStart(2,"0"),s=String(a.getDate()).padStart(2,"0");return`${t}-${o}-${s}`}function ha(a){if(!a)return"Sin fecha";const[t,o,s]=a.split("-");return`${s}/${o}/${t}`}async function ba(a){try{const t=new Date().toISOString().slice(0,7);x=t,e("#monthSelector").val(t),e("#fechaTour").val(ia()),await Promise.all([na(),B(),ra(),ca(),sa()]),Ea(),L(),V()}catch(t){console.error("Error inicializando dashboard:",t),l("Error cargando datos del dashboard","error")}}async function na(){try{const a=O("empleadosSmile");a&&(y=a,I());const t=Y(N(m,"smiles"),Q("participa","==","si"));y=(await D(t)).docs.map(s=>({id:s.id,...s.data()})),A("empleadosSmile",y,300),await R(),I()}catch(a){console.error("Error cargando empleados:",a),e("#workersGrid").html(`
            <div class="error-workers">
                <i class="fas fa-exclamation-triangle"></i>
                Error cargando empleados
            </div>
        `)}}async function R(){try{const t=(await D(N(m,"registrosdb"))).docs.filter(o=>{const s=o.data();return s.fechaTour&&s.fechaTour.startsWith(x)});y.forEach(o=>{const s=t.filter(n=>n.data().vendedor===o.usuario);o.totalPuntos=s.reduce((n,r)=>n+(r.data().puntos||0),0),o.totalVentas=s.reduce((n,r)=>n+(r.data().qventa||0),0)}),y.sort((o,s)=>s.totalPuntos-o.totalPuntos)}catch(a){console.error("Error calculando puntos:",a)}}function I(){const a=y.map((t,o)=>{const s=o+1,n=s===1,r=s===2;return`
        ...
        `)}}function S(a="",t=!1){let o=[...b];if(o=o.filter(i=>i.fechaTour&&i.fechaTour.startsWith(x)),a&&(o=o.filter(i=>i.vendedor===a)),t){const i=new Date().toISOString().split("T")[0];o=o.filter(c=>c.fechaTour===i)}const s=Math.ceil(o.length/G),n=(h-1)*G,E=o.slice(n,n+G).map(i=>{const f=i.vendedor===w?.displayName?`<button class="btn-view" onclick="verDetalleVenta('${i.id}')" title="Ver detalles"><i class="fas fa-eye"></i></button>
        ...
        `}).join("");e("#salesTableBody").html(E||'<tr><td colspan="11" class="empty-cell"><i class="fas fa-inbox"></i> No hay ventas para mostrar</td></tr>'),ya(s)}function ga(a){return{pagado:"PAGADO",cobrado:"PAGADO",cobrar:"DEUDA"}[a]||"DEUDA"}function ya(a){if(a<=1){e("#paginationContainer").html("");return}let t='<div class="pagination">';h>1&&(t+=`<button class="page-btn" onclick="cambiarPagina(${h-1})">
        ...
        </button>`),t+="</div>",e("#paginationContainer").html(t)}async function ra(){try{const a=Ta(x),t=`${a.replace("-","")}`,o=await va($(m,"ganadores",t));if(o.exists()){ta(o.data());return}const s=await D(N(m,"registrosdb")),n={};s.docs.forEach(P=>{const g=P.data();if(g.fechaTour?.startsWith(a)){const v=g.vendedor;n[v]||(n[v]={puntos:0,ventas:0}),n[v].puntos+=g.puntos||0,n[v].ventas+=g.qventa||0}});const r=Object.entries(n);if(r.length===0){e("#lastWinner").html(`
            ...
            `);return}r.sort((P,g)=>g[1].puntos-P[1].puntos);const[E,i]=r[0],[c,f]=a.split("-"),T=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],d={ganador:E,puntosGanados:i.puntos,totalVentas:i.ventas,mes:T[parseInt(f)-1],year:c,mesCompleto:a,fechaRegistro:_()};await H($(m,"ganadores",t),d),ta(d)}catch(a){console.error("Error:",a),e("#lastWinner").html(`
            <div class="winner-header">
                <i class="fas fa-trophy"></i>
                <h3>Ganador del Mes Anterior</h3>
            </div>
