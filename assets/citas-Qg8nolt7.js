import{$ as a}from"./vendor-PbmUQHyn.js";import{db as _}from"./firebase-QWwQUjRS.js";import{j as x,s as H,c as G,b as f,E as S,g as N,d as W,G as g,u as X,C as J}from"./index-BQyVSIlW.js";import{d as Q,e as $,z as O,t as M,u as V,c as P,p as C,y as q,T as K,n as Y,q as Z,i as aa}from"./firebase-DzkkQdMV.js";const b="wicitas",B="wiFrases";let l=[],w=[],n=0,r=null,E=null,F=!0,y=[],k=!1;const D=5,R=10,z=5,sa=t=>t?t[0].toUpperCase()+t.slice(1).toLowerCase():"",p=t=>t?.actualizado?.seconds||t?.creado?.seconds||0,ia=t=>(i,s)=>t==="favoritas"?s.favorito-i.favorito||p(s)-p(i):t==="recientes"?p(s)-p(i):p(i)-p(s),j=t=>[...new Set(t.map(i=>i.categoria).filter(Boolean))].map(i=>({nombre:i,count:t.filter(s=>s.categoria===i).length})).sort((i,s)=>s.count-i.count),L=()=>{const t=y.slice(0,k?y.length:z);a(".categorias_list").html(`<button class="cat_btn active" data-cat="todas"><i class="fas fa-layer-group"></i> Todas <span>(${l.length})</span></button>`+t.map(i=>`<button class="cat_btn" data-cat="${i.nombre}">${i.nombre} <span>(${i.count})</span></button>`).join("")),a(".btn_ver_mas").toggle(y.length>z).html(`<i class="fas fa-chevron-${k?"up":"down"}"></i> ${k?"Menos":"Más"}`)},I=(t,i,s=!1)=>{const c=i<10&&!s?(i*.05).toFixed(2):0,e=!!r?.email&&r.email===t.email;return`
  ...
  </div>`},A=(t=!1)=>{const i=n===0?R:D,s=w.slice(n,n+i).map((c,e)=>I(c,n+e,!1)).join("");t?a("#phrsGrid").append(s):a("#phrsGrid").html(s||'<div class="frases_empty"><i class="fas fa-seedling"></i><p>No se encontraron frases.</p></div>'),n+=i,n<w.length?(a(".load_more_section").show(),a("#btnLoadMore").off("click").on("click",()=>{A(!0),setTimeout(()=>a(".wi_fadeUp:not(.visible)").addClass("visible"),50)})):F?(a(".load_more_section").show(),a("#btnLoadMore").off("click").on("click",function(){u(this,!0)})):a(".load_more_section").hide()},T=(t=!1)=>{const i=a("#citasBusq").val().toLowerCase(),s=a(".cat_btn.active").data("cat")||"todas",c=a("#citasOrden").val()||"recientes";if(w=l.filter(e=>(s==="todas"||e.categoria===s)&&(e.cita.toLowerCase().includes(i)||e.libro.toLowerCase().includes(i))).sort(ia(c)),!t)n=0,A();else{const e=n,m=e+D,d=w.slice(0,m).map((h,v)=>I(h,v,v<e)).join("");a("#phrsGrid").html(d||'<div class="frases_empty"><i class="fas fa-seedling"></i><p>No se encontraron frases.</p></div>'),n=m,n<w.length?(a(".load_more_section").show(),a("#btnLoadMore").off("click").on("click",()=>{A(!0),setTimeout(()=>a(".wi_fadeUp:not(.visible)").addClass("visible"),50)})):F?(a(".load_more_section").show(),a("#btnLoadMore").off("click").on("click",function(){u(this,!0)})):a(".load_more_section").hide()}},ra=()=>`
<div class="wicitas">
