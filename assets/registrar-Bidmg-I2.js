import{r as e}from"./vendor-CZ6bxb2j.js";import{C as t,f as n,h as r,i,m as a,v as o}from"./widev-BkR2Na_W.js";import{n as s}from"./index-qz4bSTQe.js";import{A as c,N as l,O as u,S as d,_ as f}from"./firebase-BXqel3Di.js";import{n as p}from"./firebase-Cc-Gk9nK.js";import{cargarTours as m,getMesActual as h,invalidateRankingCaches as g,obtenerRankingMes as _}from"./zsmile-DFnR68aQ.js";var v=[],y=null,b=null,x=!1,S=()=>`
...
`,C=async()=>{let t=o.user;if(!t)return setTimeout(()=>s.navigate(`/login`),100);let r=e(`#smwRegCard`);r.addClass(`smw_loading`),e(`#tourDisplay`).addClass(`smw_loading_select`),e(`#formularioVenta`).removeClass(`smw_show_checks`);let i=[`Enero`,`Febrero`,`Marzo`,`Abril`,`Mayo`,`Junio`,`Julio`,`Agosto`,`Septiembre`,`Octubre`,`Noviembre`,`Diciembre`],[a,c]=h().split(`-`);e(`#lblMesActual`).text(`${i[parseInt(c)-1]} ${a}`);let l=n(`toursSmile`);l?.length>0&&(v=l.map(e=>({nt:e.num||Math.random(),tour:e.tour,price:parseFloat(e.precio)||0,pts:parseInt(e.puntos)||0,com:parseFloat(e.comision)||5})),T(v)),m().then(e=>{v=e,T(v),L(t,!0),V(!0),F()}).catch(e=>console.error(`Error al cargar tours:`,e)).finally(()=>{r.removeClass(`smw_loading`),e(`#tourDisplay`).removeClass(`smw_loading_select`)}),E().catch(e=>console.error(e)),O(t.usuario).catch(e=>console.error(e)),A(),L(t),V(),F();for(let e of[100,300,600,1e3,1800,3e3])setTimeout(F,e)},w=()=>{e(document).off(`.registrar`),y=null,v=[],b=null,x=!1};function T(t){if(!t.length){e(`#tourTableBody`).html(`<tr><td colspan="4" style="text-align:center;color:var(--tx3);padding:2vh;">Sin resultados para la búsqueda</td></tr>`);return}let n=t.map((e,t)=>`
  ...
  `).join(``);e(`#tourTableBody`).html(n)}async function E(){try{let t=h();n(`empleadosPuntos_${t}`)||e(`#miniRankingList`).html(z(3));let r=await _(t);if(!r||!r.length){e(`#miniRankingList`).html(`
    ...
    `)}}async function D(e,t){try{let i=n(`svVentas`);if(i&&i.vendedor===e&&i.mes===t)return console.log(`⚡ [Smart Cache] Mis ventas recuperadas usando la clave svVentas.`),i.ventas;let a=`ventasSmile_${e}_${t}`,o=n(a);if(o)return o;let[s,c]=t.split(`-`).map(Number),l=await f(u(p,`registrosdb`)),d=[];return l.docs.forEach(t=>{let n=t.data();if(n.vendedor!==e)return;let r=n.fechaTour,i,a;if(typeof r==`string`)[i,a]=r.split(`-`).map(Number);else if(r?.toDate){let e=r.toDate();i=e.getFullYear(),a=e.getMonth()+1}else return;i===s&&a===c&&d.push({idVenta:n.idVenta||t.id,...n})}),d.sort((e,t)=>{let n=e.fechaRegistro?.seconds?e.fechaRegistro.seconds*1e3:e.fechaRegistro?.toDate?e.fechaRegistro.toDate().getTime():Date.now();return(t.fechaRegistro?.seconds?t.fechaRegistro.seconds*1e3:t.fechaRegistro?.toDate?t.fechaRegistro.toDate().getTime():Date.now())-n}),r(a,d,10),r(`svVentas`,{vendedor:e,mes:t,ventas:d},10),d}catch(e){return console.error(`Error al obtener mis ventas del mes:`,e),[]}}async function O(t){try{let n=await D(t,h()),r=n.reduce((e,t)=>e+(parseInt(t.puntos)||0),0);e(`#lblMisPuntosMes`).text(`${r} pts`);let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,`0`)}-${String(i.getDate()).padStart(2,`0`)}`,o=n.filter(e=>{let t=e.fechaTour;return t&&t.toDate&&(t=t.toDate().toISOString().split(`T`)[0]),t===a});if(!o.length){e(`#myTodaySalesList`).html(`
        <div class="smw_mini_empty">
