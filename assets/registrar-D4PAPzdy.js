import{$ as a}from"./vendor-2D3jvCpt.js";import{db as J}from"./firebase-DXGRMKXG.js";import{v as U,e as G,i as z,c as B,u as Y}from"./firebase-BwR1K4LJ.js";import{y as R,s as L,j as w,r as S,b as _,E as A,u as M}from"./index-CGMM99MM.js";import{getMesActual as V,cargarTours as K,obtenerRankingMes as X,invalidateRankingCaches as Z}from"./zsmile-Bgl22WoR.js";let f=[],p=null,b=null,y=!1;const ia=()=>`
...
`,ra=async()=>{const t=R.user;if(!t)return setTimeout(()=>L.navigate("/login"),100);const e=a("#smwRegCard");e.addClass("smw_loading"),a("#tourDisplay").addClass("smw_loading_select");const s=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],o=V(),[l,i]=o.split("-");a("#lblMesActual").text(`${s[parseInt(i)-1]} ${l}`);const n=w("toursSmile");n?.length>0&&(f=n.map(r=>({nt:r.num||Math.random(),tour:r.tour,price:parseFloat(r.precio)||0,pts:parseInt(r.puntos)||0,com:parseFloat(r.comision)||5})),T(f)),K().then(r=>{f=r,T(f),O(t,!0),I(!0),v()}).catch(r=>console.error("Error al cargar tours:",r)).finally(()=>{e.removeClass("smw_loading"),a("#tourDisplay").removeClass("smw_loading_select")}),N().catch(r=>console.error(r)),q(t.usuario).catch(r=>console.error(r)),Q(),O(),I(),v();for(let r of[100,300,600,1e3,1800,3e3])setTimeout(v,r)},la=()=>{a(document).off(".registrar"),p=null,f=[],b=null,y=!1};function T(t){if(!t.length){a("#tourTableBody").html('<tr><td colspan="4" style="text-align:center;color:var(--tx3);padding:2vh;">Sin resultados para la búsqueda</td></tr>');return}const e=t.map((s,o)=>`
  ...
  `).join("");a("#tourTableBody").html(e)}async function N(){try{const t=V(),e=`empleadosPuntos_${t}`;w(e)||a("#miniRankingList").html(H(3));const o=await X(t);if(!o||!o.length){a("#miniRankingList").html(`
        <div class="smw_mini_empty">
    ...
    `)}}async function j(t,e){try{const s=w("svVentas");if(s&&s.vendedor===t&&s.mes===e)return console.log("⚡ [Smart Cache] Mis ventas recuperadas usando la clave svVentas."),s.ventas;const o=`ventasSmile_${t}_${e}`,l=w(o);if(l)return l;const[i,n]=e.split("-").map(Number),r=await z(B(J,"registrosdb")),c=[];return r.docs.forEach(d=>{const m=d.data();if(m.vendedor!==t)return;const u=m.fechaTour;let k,D;if(typeof u=="string")[k,D]=u.split("-").map(Number);else if(u?.toDate){const F=u.toDate();k=F.getFullYear(),D=F.getMonth()+1}else return;k===i&&D===n&&c.push({idVenta:m.idVenta||d.id,...m})}),c.sort((d,m)=>{const u=d.fechaRegistro?.seconds?d.fechaRegistro.seconds*1e3:d.fechaRegistro?.toDate?d.fechaRegistro.toDate().getTime():Date.now();return(m.fechaRegistro?.seconds?m.fechaRegistro.seconds*1e3:m.fechaRegistro?.toDate?m.fechaRegistro.toDate().getTime():Date.now())-u}),M(o,c,10),M("svVentas",{vendedor:t,mes:e,ventas:c},10),c}catch(s){return console.error("Error al obtener mis ventas del mes:",s),[]}}async function q(t){try{const e=V(),s=await j(t,e),o=s.reduce((c,d)=>c+(parseInt(d.puntos)||0),0);a("#lblMisPuntosMes").text(`${o} pts`);const l=new Date,i=`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}-${String(l.getDate()).padStart(2,"0")}`,n=s.filter(c=>{let d=c.fechaTour;return d&&d.toDate&&(d=d.toDate().toISOString().split("T")[0]),d===i});if(!n.length){a("#myTodaySalesList").html(`
        <div class="smw_mini_empty">
  ...
  `).join("")}function H(t=3){return Array(t).fill(0).map((e,s)=>`
