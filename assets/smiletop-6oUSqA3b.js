import{t as it,B as I,b as P,q as L,c as C,a as w,e as S,N as i,v as W,h as U,l as V,$ as e,F as rt,C as M,G as Y,A as H,z as ct,d as N,D as x,j as _,H as X,i as J}from"./widev-9nBR2YtD.js";let m=null,v=new Date().toISOString().slice(0,7),g="todos",d=0,T=0,b=[],y=[],u=[],F=7,p=[];it(U,async t=>{if(!t)return window.location.href="/";try{const o=I("wiSmileTop");if(o)return await q(o);const a=await P(L(C(w,"smiles"),S("usuario","==",t.displayName)));if(a.empty||a.docs[0].data().rol!=="smiletop")return i("No tienes permisos de administrador","error"),await W(U);const s=a.docs[0].data();V("wiSmileTop",s,450),await q(s)}catch(o){console.error("Error auth:",o),i("Error al cargar aplicación","error")}});async function q(t){m=t,console.log(`✅ Admin: ${t.nombre}`),e(".app").html(lt()),rt();try{await K(),await k(),await Q(),await A(),ft(),E(),$(),i("Dashboard cargado","success")}catch(o){console.error("Error init:",o),i("Error al inicializar","error")}}function lt(){return`
    ...
    `;t.html(o);const a=b.reduce((n,r)=>n+(parseFloat(r.importeTotal)||0),0),s=b.reduce((n,r)=>n+(parseInt(r.puntos)||0),0);O(b,a,s),ut()}function Z(t,o=!1){const a=y.find(c=>c.usuario===t.vendedor||c.id===t.vendedor),s=t.fechaTour?new Date(t.fechaTour).toLocaleDateString("es-ES"):"Sin fecha",n=parseFloat(t.precioUnitario)||0,r=parseFloat(t.importeTotal)||0,l=parseFloat(t.comision)||r*.1,f=parseFloat(t.ganancia)||r-l,h=parseInt(t.puntos)||0;return o?`
