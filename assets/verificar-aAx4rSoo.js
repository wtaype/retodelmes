...
`,C=null,w=null,T=60;function E(){w&&clearInterval(w);let e=localStorage.getItem(`vault_expire`);e||(e=Date.now()+T*1e3,localStorage.setItem(`vault_expire`,e));let n=()=>Math.max(0,Math.ceil((parseInt(e)-Date.now())/1e3)),i=n();t(`#vault_timer`).text(`${i}s`);let o=async()=>{if(i=n(),t(`#vault_timer`).text(`${i}s`),i<=0){w&&clearInterval(w),w=null,localStorage.removeItem(`vault_expire`),r(`Sesión cerrada por inactividad`,`error`);let{salir:e}=await a(async()=>{let{salir:e}=await import(`./login-J-UFsb88.js`);return{salir:e}},__vite__mapDeps([0]));await e()}};o(),i>0&&(w=setInterval(o,1e3))}var D=()=>{let e=n(`wiSmile`);return!e||e.rol!==`admin`?``:`
