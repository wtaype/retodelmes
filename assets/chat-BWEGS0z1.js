import{r as e}from"./vendor-CZ6bxb2j.js";import{C as t,f as n,h as r,i,n as a,v as o,w as s}from"./widev-BkR2Na_W.js";import{a as c,c as l,d as u,g as d,l as f,n as p,o as m,t as ee,v as h,x as g}from"./firebase-D6VL4aaK.js";import{n as _}from"./firebase-BfEAzf30.js";import{cargarTodosEmpleados as v}from"./zsmile-9OBMck8o.js";var y=`.chat`,b=`chatSmileMsgs`,x=.1,S=7,C=5,w=3e4,T=500,E=[],D=[],O=``,k=null,A=!1,j=``,M=``,N=!1,P=!0,F=null,te=()=>`
...
`;function I(){return[,,,,,].fill(0).map(()=>`
  ...
  `,U=e=>String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`),ne=e=>e.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),W=()=>{let t=e(`#chatTextarea`).val()||``;e(`.chat_sidebar_item`).each(function(){let n=(e(this).attr(`data-nombre`)||``).trim();n&&(RegExp(`@`+ne(n)+`(?![a-zA-Z0-9áéíóúüñÁÉÍÓÚÜÑ])`,`i`).test(t)?e(this).addClass(`active`):e(this).removeClass(`active`))})},G=(t=!1)=>{let n=e(`#chatMessages`);if(!E.length){n.html(`
    ...
    `);return}let r=n.map(e=>{let t=e.usuario===j,n=R(e.nombre||`?`),r=B(e.usuario||``),i=e.estado===`activo`,o=e.imagen||e.avatar,s=o?`<img class="chat_sidebar_avatar_img" src="${o}" alt="${e.nombre}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">`:``;return`${e.nombre||``} ${e.apellidos||``}`.replace(/\s+/g,` `).trim(),`
    ...
    `}).join(``);t.html(r),W()},q=(e=!1)=>{let t=document.getElementById(`chatMessages`);t&&t.scrollTo({top:t.scrollHeight,behavior:e?`smooth`:`auto`})},J=t=>{e(`#chatOnlineCount`).text(t)},Y=async(t=!1)=>{if(!N){if(N=!0,!t){let e=n(b);if(e&&Array.isArray(e)){E=e,P=!0,G(),N=!1;return}}t&&e(`#chatRefresh`).addClass(`chat_spinning`),e(`#chatHeader`).addClass(`smw_loading`);try{let e=await c(f(d(_,`chatSmiles`),l(`creado`,`desc`),m(S)));E=e.docs.reverse().map(e=>({id:e.id,...e.data()})),P=e.size>=S,r(b,E,x),G(),J(new Set(E.map(e=>e.usuario||e.autor).filter(Boolean)).size)}catch(t){console.error(`[Chat] loadMensajes error:`,t),E.length||e(`#chatMessages`).html(`
      ...
      `),i(`No se pudieron cargar los mensajes`,`error`)}finally{N=!1,e(`#chatHeader`).removeClass(`smw_loading`),e(`#chatRefresh`).removeClass(`chat_spinning`)}}},X=async()=>{if(N||!P||!E.length)return;let t=e(`#chatMessages`),n=E[0].creado||E[0].ts;if(!n){i(`No se puede paginar: falta cursor de tiempo`,`warning`);return}N=!0,e(`#chatPaginateBtn`).prop(`disabled`,!0).html(`<i class="fas fa-circle-notch fa-spin"></i> Cargando anteriores...`);try{let e=await c(f(d(_,`chatSmiles`),l(`creado`,`desc`),u(n),m(C)));if(e.empty){P=!1,G(!0),i(`No hay más mensajes anteriores`,`info`);return}let r=e.docs.reverse().map(e=>({id:e.id,...e.data()}));r.length<C&&(P=!1);let a=t[0].scrollHeight,o=t[0].scrollTop;E=[...r,...E],G(!0);let s=t[0].scrollHeight;t[0].scrollTop=o+(s-a),i(`Cargados ${r.length} mensajes anteriores`,`success`)}catch(e){console.error(`[Chat] loadAnteriores error:`,e),i(`Error al cargar anteriores`,`error`)}finally{N=!1}},Z=async()=>{try{D=await v(!0),K()}catch(e){console.error(`[Chat] _loadColaboradores error:`,e)}},Q=async()=>{if(!A)return;let t=e(`#chatTextarea`),n=t.val().trim();if(!n)return;if(n.length>T){i(`El mensaje excede ${T} caracteres`,`warning`);return}t.val(``).trigger(`input`),$(t[0]);let a=`temp_`+Date.now()+Math.random().toString(36).substr(2,5),s=o.user,c=s?.imagen||s?.avatar||``,l={id:a,texto:n,usuario:j,email:s?.email||``,nombre:M,imagen:c,creado:new Date,ts:new Date,tipo:`texto`,temp:!0};E.push(l),G(!1),ee(d(_,`chatSmiles`),{texto:n,usuario:j,email:s?.email||``,nombre:M,imagen:c,creado:g(),ts:g(),tipo:`texto`}).then(e=>{let t=E.findIndex(e=>e.id===a);t!==-1&&(E[t].id=e.id,delete E[t].temp,r(b,E,x),G(!0))}).catch(e=>{console.error(`[Chat] Background send error:`,e),i(`No se pudo entregar el mensaje`,`error`),E=E.filter(e=>e.id!==a),r(b,E,x),G(!0)})},$=e=>{e&&(e.style.height=`auto`,e.style.height=Math.min(e.scrollHeight,96)+`px`,e.style.overflowY=e.scrollHeight>96?`auto`:`hidden`)},re=()=>{let t=e(`#chatTextarea`),n=e(`#chatSendBtn`),r=e(`#chatBlockedMsg`);A?(t.prop(`disabled`,!1).attr(`placeholder`,`Escribe un mensaje…`),n.prop(`disabled`,!1).removeClass(`chat_send_disabled`),r.hide()):(t.prop(`disabled`,!0).attr(`placeholder`,`No puedes enviar mensajes (participación inactiva)`),n.prop(`disabled`,!0).addClass(`chat_send_disabled`),r.show())},ie=async()=>{e(`.chat_wrap`).addClass(`visible`),window.__WIREADY__=!0;let n=o.user;n&&(j=n.usuario||n.email||``,M=n.nombre||n.usuario||`Colaborador`,A=n.participa===`si`),re(),Y(!1),Z(),k=setInterval(()=>{r(b,null,0),Y(!0),Z()},w),e(document).on(`input${y}`,`#chatTextarea`,function(){$(this),W()}),e(document).on(`keydown${y}`,`#chatTextarea`,function(e){e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),Q())}),e(document).on(`click${y}`,`#chatSendBtn`,()=>Q()),e(document).on(`click${y}`,`#chatRefresh`,async function(){r(b,null,0),P=!0,await Y(!0),await Z(),i(`Mensajes actualizados`,`success`)}),e(document).on(`click${y}`,`#chatPaginateBtn`,()=>X()),e(document).on(`click${y}`,`.chat_msg_delete_btn`,function(){let t=e(this).attr(`data-id`);t&&(F=t,e(`#chatEliminarModal`).addClass(`show`))}),e(document).on(`click${y}`,`#chatCancelDeleteBtn, #chatEliminarModal`,function(t){e(t.target).is(`#chatCancelDeleteBtn, #chatEliminarModal`)&&(e(`#chatEliminarModal`).removeClass(`show`),F=null)}),e(document).on(`click${y}`,`#chatConfirmDeleteBtn`,async function(){if(!F)return;let n=this;t(n,!0,`Eliminando...`);try{let t=F;await p(h(_,`chatSmiles`,t)),i(`Mensaje eliminado permanentemente`,`success`),e(`#chatEliminarModal`).removeClass(`show`),F=null;let n=e(`.chat_bubble_wrap[data-id="${t}"]`);n.length?n.fadeOut(300,function(){e(this).remove(),E=E.filter(e=>e.id!==t),r(b,E,x)}):(E=E.filter(e=>e.id!==t),r(b,E,x),G(!0))}catch(e){console.error(`[Chat] deleteDoc error:`,e),i(`Error al eliminar mensaje`,`error`)}finally{t(n,!1,`Eliminar`)}}),e(document).on(`input${y}`,`#chatSearchInput`,function(){O=e(this).val(),K()}),e(document).on(`click${y}`,`.chat_sidebar_item`,function(t){if(!A)return;let n=(e(this).attr(`data-nombre`)||``).trim(),r=e(`#chatTextarea`),i=e(this).hasClass(`active`),a=r.val(),o=`@${n}`;if(i){let e=n.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),t=RegExp(`@`+e+`(?![a-zA-Z0-9áéíóúüñÁÉÍÓÚÜÑ])\\s*`,`gi`);a=a.replace(t,``),a=a.replace(/\s+/g,` `).trim(),r.val(a?a+` `:``)}else{let e=a?a.endsWith(` `)?``:` `:``;r.val(a+e+o+` `)}r.trigger(`input`),r.focus(),$(r[0]),e(`#chatContainer`).removeClass(`chat_sidebar_active`)}),e(document).on(`click${y}`,`#chatSidebarToggle`,function(){e(`#chatContainer`).addClass(`chat_sidebar_active`)}),e(document).on(`click${y}`,`#chatSidebarClose`,function(){e(`#chatContainer`).removeClass(`chat_sidebar_active`)}),e(document).on(`click${y}`,`#chatRetry`,()=>{e(`#chatMessages`).html(L()),e(`#chatSidebarList`).html(I()),Y(!0),Z()}),e(document).on(`focus${y}`,`#chatTextarea`,function(){setTimeout(()=>q(!0),350)})},ae=()=>{e(document).off(y),clearInterval(k),k=null,E=[],D=[],O=``,A=!1,j=``,M=``,N=!1,P=!0,F=null};export{ae as cleanup,ie as init,te as render};