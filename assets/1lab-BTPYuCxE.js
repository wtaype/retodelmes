import"./wiad-DWbEudQX.js";import{$ as i}from"./vendor-PbmUQHyn.js";import{y as v,l as r,x as w,F as b,c as k,I as o,A as y,e as x,n as S}from"./index-q-muxlkU.js";import{k as u,i as A,l as f,m as I,b as P,o as C,t as L}from"./devblog-V8k2W-GN.js";import"./firebase-oqHtqqC9.js";import"./firebase-BM1KOhEp.js";const h=s=>`<a href="/${s.slug}" class="po_rel_card" ${o(s.resumen||s.titulo)}><div class="po_rel_img"><img src="${s.imagen}" alt="${s.imagenAlt||s.titulo}" loading="lazy"/></div><div class="po_rel_info"><span class="po_rel_cat"><i class="fas fa-paw"></i> ${s.categoria}</span><strong>${s.titulo}</strong><span class="po_rel_meta"><i class="fas fa-clock"></i> ${s.tiempoLectura} · <i class="fas fa-eye"></i> ${s.vistas||0} . <i class="fas fa-heart"></i> ${s.likes||0}</span></div></a>`,g=(s,d,_,e,a)=>{const c=y.user?.usuario,l=C(s.creado,!0),n=(s.tags||[]).map(p=>`<span class="po_tag">#${p}</span>`).join(""),t=(s.vistas||0)+1;return`
    ...
    <div class="po_wrap"><div class="po_layout">
      <div class="po_col_main">
        ...
        ${n?`<div class="po_tags" data-showi="200">${n}</div>`:""}
    ...
    <aside class="po_sidebar">
        <div class="po_side_card" data-showi="150">
          ...
          <div class="po_autor_box"><div class="po_autor_av"><img src="/smile.avif" alt="${s.autor}"/></div><div class="po_autor_info"><strong>${s.autor}</strong><span>${x} <i class="fas ${S}"></i></span></div></div>
    ...
    </div></div>`},j=()=>{const s="amar_como_dios_manda_tiempos_dificiles",d=r(`wi_post_${s}`),_=r(`wi_mas_${s}`),e=r("wi_sidebar_posts");if(d&&_&&e)return'<div id="po_instant_container"></div>';const a=u(s);return a?`
    <div class="po_wrap"><div class="po_layout">
      <div class="po_col_main">
        <div class="po_content">
    ...
    </div></div>`:`<div class="po_wrap"><div class="po_layout"><div class="po_col_main"><div class="po_content"><div class="po_sk_img shimmer"></div><div class="po_sk_body"><div class="po_sk_cat shimmer"></div><div class="po_sk_tit shimmer"></div><div class="po_sk_tit po_sk_t2 shimmer"></div><div class="po_sk_meta shimmer"></div>${'<div class="po_sk_p shimmer"></div>'.repeat(5)}</div></div></div><aside class="po_sidebar">${'<div class="po_sk_side shimmer"></div>'.repeat(3)}</aside></div></div>`},q=async()=>{const s="amar_como_dios_manda_tiempos_dificiles";let d=!0;(()=>{const a=r(`wi_post_${s}`),c=r(`wi_mas_${s}`),l=r("wi_sidebar_posts");return a&&c&&l?(i("#po_instant_container").replaceWith(g(a,c,l,s,!0)),d=!1,!0):!1})(),v();const e=()=>{const a=i(".po_contenido").find("h2, h3");if(a.length>0&&!i(".po_toc_box").length){let c='<div class="po_toc_box" data-showi="180"><div class="po_toc_title"><i class="fas fa-list"></i> En este artículo</div><ul class="po_toc">';a.each((l,n)=>{const t="po_h_"+l;i(n).attr("id",t).css("scroll-margin-top","7vh"),c+=`<li style="margin-left:${n.tagName.toLowerCase()==="h3"?"1.5vh":"0"}"><a href="#${t}" class="po_toc_link">${i(n).text()}</a></li>`}),i(c+"</ul></div>").insertBefore(".po_contenido"),v()}i(window).off("scroll.post_prog").on("scroll.post_prog",()=>i("#po_progress").css("width",(i(window).scrollTop()/(i(document).height()-i(window).height())*100||0)+"%"))};d||e();try{const a=u(s),[c,l,n]=await Promise.all([A(s,!1),a?f(s,a.categoria,!1):Promise.resolve([]),I(s,!1)]);if(!c?.data?.activo)return i("#wimain").html('<div class="po_err dpvc"><i class="fas fa-paw"></i><h2>Historia no encontrada</h2><p>No existe o no está disponible 🐾</p><a href="/blog" class="po_back_btn"><i class="fas fa-arrow-left"></i> Ver historias</a></div>');const{data:t,fromCache:p}=c;p||P(s);const $=l.length?l:await f(s,t.categoria,!1);d||JSON.stringify(t.contenido)!==JSON.stringify(r(`wi_post_${s}`)?.contenido)?(i("#wimain").html(g(t,$,n,s,p)),e(),v()):p||i(".po_cache_badge").remove(),w({title:t.titulo,desc:t.resumen,keywords:t.keywords,img:t.imagenTop||t.imagen,path:`/${s}`,type:"Article",datePublished:t.creado});const m=function(){this.page.url=`https://wiihope.com/${s}`,this.page.identifier=s,this.page.title=t.titulo};window.DISQUS?window.DISQUS.reset({reload:!0,config:m}):(window.disqus_config=m,b({disqus:[async()=>i("body").append(i("<script>",{src:"https://superwii.disqus.com/embed.js","data-timestamp":+new Date}))]}))}catch(a){console.error("[post]",a),k("Error al cargar","error")}},z=()=>i(document).off(".post");export{z as cleanup,q as init,j as render};
