import{$ as e}from"./vendor-2D3jvCpt.js";import{db as A}from"./firebase-BPYZb32v.js";import{d as ta,e as ea,r as z,c as I,q as G,n as P,i as F,a as sa,z as ia,u as L}from"./firebase-BwR1K4LJ.js";import{y as S,u as v,b as _,E as N,j as na,a as k,F as K}from"./index-Ds5QDkyH.js";import{cargarTodosEmpleados as ca}from"./zsmile-D_ugFjjn.js";const d=".chat",u="chatSmileMsgs",C=.1,T=7,O=5,ra=3e4,j=500;let n=[],R=[],D="",H=null,w=!1,f="",M="",m=!1,p=!0,$=null;const ga=()=>`
...
`;function W(){return Array(5).fill(0).map(()=>`
  ...
  `},la=a=>{const t=K(a.creado||a.ts),s=X(a.texto||"");return`
  ...
  `},X=a=>String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),da=a=>a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),J=()=>{const a=e("#chatTextarea").val()||"";e(".chat_sidebar_item").each(function(){const t=(e(this).attr("data-nombre")||"").trim();if(!t)return;new RegExp("@"+da(t)+"(?![a-zA-Z0-9áéíóúüñÁÉÍÓÚÜÑ])","i").test(a)?e(this).addClass("active"):e(this).removeClass("active")})},b=(a=!1)=>{const t=e("#chatMessages");if(!n.length){t.html(`
    ...
    `);return}const s=t.map(i=>{const r=i.usuario===f,c=V(i.nombre||"?"),l=Z(i.usuario||""),o=i.estado==="activo",h=i.imagen||i.avatar,g=h?`<img class="chat_sidebar_avatar_img" src="${h}" alt="${i.nombre}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">`:"";return`${i.nombre||""}${i.apellidos||""}`,`
      <div class="chat_sidebar_item ${r?"chat_sidebar_item_me":""}" data-usuario="${i.usuario}" data-nombre="${i.usuario}">
    ...
    `}).join("");a.html(s),J()},aa=(a=!1)=>{const t=document.getElementById("chatMessages");t&&t.scrollTo({top:t.scrollHeight,behavior:a?"smooth":"auto"})},ha=a=>{e("#chatOnlineCount").text(a)},E=async(a=!1)=>{if(!m){if(m=!0,!a){const t=na(u);if(t&&Array.isArray(t)){n=t,p=!0,b(),m=!1;return}}a&&e("#chatRefresh").addClass("chat_spinning"),e("#chatHeader").addClass("smw_loading");try{const t=z(I(A,"chatSmiles"),G("creado","desc"),P(T)),s=await F(t);n=s.docs.reverse().map(c=>({id:c.id,...c.data()})),p=s.size>=T,v(u,n,C),b();const r=new Set(n.map(c=>c.usuario||c.autor).filter(Boolean));ha(r.size)}catch(t){console.error("[Chat] loadMensajes error:",t),n.length||e("#chatMessages").html(`
      ...
      `),_("No se pudieron cargar los mensajes","error")}finally{m=!1,e("#chatHeader").removeClass("smw_loading"),e("#chatRefresh").removeClass("chat_spinning")}}},_a=async()=>{if(m||!p||!n.length)return;const a=e("#chatMessages"),t=n[0].creado||n[0].ts;if(!t){_("No se puede paginar: falta cursor de tiempo","warning");return}m=!0,e("#chatPaginateBtn").prop("disabled",!0).html('<i class="fas fa-circle-notch fa-spin"></i> Cargando anteriores...');try{const i=z(I(A,"chatSmiles"),G("creado","desc"),ia(t),P(O)),r=await F(i);if(r.empty){p=!1,b(!0),_("No hay más mensajes anteriores","info");return}const l=r.docs.reverse().map(y=>({id:y.id,...y.data()}));l.length<O&&(p=!1);const o=a[0].scrollHeight,h=a[0].scrollTop;n=[...l,...n],b(!0);const g=a[0].scrollHeight;a[0].scrollTop=h+(g-o),_(`Cargados ${l.length} mensajes anteriores`,"success")}catch(i){console.error("[Chat] loadAnteriores error:",i),_("Error al cargar anteriores","error")}finally{m=!1}},x=async()=>{try{R=await ca(!0),Q()}catch(a){console.error("[Chat] _loadColaboradores error:",a)}},U=async()=>{if(!w)return;const a=e("#chatTextarea"),t=a.val().trim();if(!t)return;if(t.length>j){_(`El mensaje excede ${j} caracteres`,"warning");return}a.val("").trigger("input"),B(a[0]);const s="temp_"+Date.now()+Math.random().toString(36).substr(2,5),i=S.user,r=i?.imagen||i?.avatar||"",c={id:s,texto:t,usuario:f,email:i?.email||"",nombre:M,imagen:r,creado:new Date,ts:new Date,tipo:"texto",temp:!0};n.push(c),b(!1),sa(I(A,"chatSmiles"),{texto:t,usuario:f,email:i?.email||"",nombre:M,imagen:r,creado:L(),ts:L(),tipo:"texto"}).then(l=>{const o=n.findIndex(h=>h.id===s);o!==-1&&(n[o].id=l.id,delete n[o].temp,v(u,n,C),b(!0))}).catch(l=>{console.error("[Chat] Background send error:",l),_("No se pudo entregar el mensaje","error"),n=n.filter(o=>o.id!==s),v(u,n,C),b(!0)})},B=a=>{if(!a)return;a.style.height="auto";const s=24*4;a.style.height=Math.min(a.scrollHeight,s)+"px",a.style.overflowY=a.scrollHeight>s?"auto":"hidden"},ua=()=>{const a=e("#chatTextarea"),t=e("#chatSendBtn"),s=e("#chatBlockedMsg");w?(a.prop("disabled",!1).attr("placeholder","Escribe un mensaje…"),t.prop("disabled",!1).removeClass("chat_send_disabled"),s.hide()):(a.prop("disabled",!0).attr("placeholder","No puedes enviar mensajes (participación inactiva)"),t.prop("disabled",!0).addClass("chat_send_disabled"),s.show())},$a=async()=>{e(".chat_wrap").addClass("visible"),window.__WIREADY__=!0;const a=S.user;a&&(f=a.usuario||a.email||"",M=a.nombre||a.usuario||"Colaborador",w=a.participa==="si"),ua(),E(!1),x(),H=setInterval(()=>{v(u,null,0),E(!0),x()},ra),e(document).on(`input${d}`,"#chatTextarea",function(){B(this),J()}),e(document).on(`keydown${d}`,"#chatTextarea",function(t){t.key==="Enter"&&!t.shiftKey&&(t.preventDefault(),U())}),e(document).on(`click${d}`,"#chatSendBtn",()=>U()),e(document).on(`click${d}`,"#chatRefresh",async function(){v(u,null,0),p=!0,await E(!0),await x(),_("Mensajes actualizados","success")}),e(document).on(`click${d}`,"#chatPaginateBtn",()=>_a()),e(document).on(`click${d}`,".chat_msg_delete_btn",function(){const t=e(this).attr("data-id");t&&($=t,e("#chatEliminarModal").addClass("show"))}),e(document).on(`click${d}`,"#chatCancelDeleteBtn, #chatEliminarModal",function(t){e(t.target).is("#chatCancelDeleteBtn, #chatEliminarModal")&&(e("#chatEliminarModal").removeClass("show"),$=null)}),e(document).on(`click${d}`,"#chatConfirmDeleteBtn",async function(){if(!$)return;const t=this;N(t,!0,"Eliminando...");try{const s=$;await ta(ea(A,"chatSmiles",s)),_("Mensaje eliminado permanentemente","success"),e("#chatEliminarModal").removeClass("show"),$=null;const i=e(`.chat_bubble_wrap[data-id="${s}"]`);i.length?i.fadeOut(300,function(){e(this).remove(),n=n.filter(r=>r.id!==s),v(u,n,C)}):(n=n.filter(r=>r.id!==s),v(u,n,C),b(!0))}catch(s){console.error("[Chat] deleteDoc error:",s),_("Error al eliminar mensaje","error")}finally{N(t,!1,"Eliminar")}}),e(document).on(`input${d}`,"#chatSearchInput",function(){D=e(this).val(),Q()}),e(document).on(`click${d}`,".chat_sidebar_item",function(t){if(!w)return;const s=(e(this).attr("data-nombre")||"").trim(),i=e("#chatTextarea"),r=e(this).hasClass("active");let c=i.val();const l=`@${s}`;if(r){const o=s.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),h=new RegExp("@"+o+"(?![a-zA-Z0-9áéíóúüñÁÉÍÓÚÜÑ])\\s*","gi");c=c.replace(h,""),c=c.replace(/\s+/g," ").trim(),i.val(c?c+" ":"")}else{const o=c?c.endsWith(" ")?"":" ":"";i.val(c+o+l+" ")}i.trigger("input"),i.focus(),B(i[0]),e("#chatContainer").removeClass("chat_sidebar_active")}),e(document).on(`click${d}`,"#chatSidebarToggle",function(){e("#chatContainer").addClass("chat_sidebar_active")}),e(document).on(`click${d}`,"#chatSidebarClose",function(){e("#chatContainer").removeClass("chat_sidebar_active")}),e(document).on(`click${d}`,"#chatRetry",()=>{e("#chatMessages").html(Y()),e("#chatSidebarList").html(W()),E(!0),x()}),e(document).on(`focus${d}`,"#chatTextarea",function(){setTimeout(()=>aa(!0),350)})},ya=()=>{e(document).off(d),clearInterval(H),H=null,n=[],R=[],D="",w=!1,f="",M="",m=!1,p=!0,$=null};export{ya as cleanup,$a as init,ga as render};
