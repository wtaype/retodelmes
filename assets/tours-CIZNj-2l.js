import{$ as s}from"./vendor-PbmUQHyn.js";import{A as r,u as l}from"./index-DKQikmxs.js";import{cargarTours as d}from"./zsmile-TFisWv2O.js";import"./firebase-DmJttupn.js";import"./firebase-BM1KOhEp.js";let e=[];const u=()=>`
...
`,w=async()=>{if(!r.user)return setTimeout(()=>l.navigate("/login"),100);c(),s(document).off("click.tours_events",".smw_tab_btn").on("click.tours_events",".smw_tab_btn",function(){const i=s(this).data("tab");s(".smw_tab_btn").removeClass("active"),s(this).addClass("active"),s(".smw_tab_pane").removeClass("active"),s(`#pane-${i}`).addClass("active")}),s(".wi_fadeUp").addClass("visible wi_visible"),window.__WIREADY__=!0},g=()=>{s(document).off(".tours_events")};async function c(){try{if(s("#preciosCatalogGrid").html(o(4)),s("#puntosCatalogGrid").html(o(4)),e=await d(),!e.length){s("#preciosCatalogGrid").html('<div class="smw_empty_pane">No hay tours cargados en la base de datos.</div>'),s("#puntosCatalogGrid").html('<div class="smw_empty_pane">No hay tours cargados en la base de datos.</div>');return}const a=e.map(t=>`
    ...
    `).join("");s("#preciosCatalogGrid").html(a);const i=e.map(t=>`
    ...
    `).join("");s("#puntosCatalogGrid").html(i)}catch(a){console.error("Error al cargar catalogo:",a),s("#preciosCatalogGrid").html('<div class="smw_empty_pane">Error al cargar información de tours.</div>')}}function o(a=4){return Array(a).fill(0).map(()=>`
  ...
  `).join("")}export{g as cleanup,w as init,u as render};
