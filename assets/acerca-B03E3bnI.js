import{r as e}from"./vendor-CZ6bxb2j.js";import{t}from"./wii-BvK6d7hI.js";import{E as n}from"./widev-D5ebok9f.js";/* empty css               */var r=()=>`
<main id="wimain">
<div class="ac_wrap">
  ...
  <section class="ac_sec">
    <div class="ac_sec_head">
      ...
      <h2 class="ac_sec_tit">El propósito de <span class="ac_grad">${t}</span></h2>
    </div>
    <div class="ac_historia wi_fadeUp">
      <p><strong>${t}</strong> es una plataforma interna exclusiva diseñada para optimizar y transparentar el registro de los tours vendidos y ejecutados por nuestro equipo de guías y agentes comerciales en la región de Ica-Huacachina.</p>
...
`,i=()=>{e(`.ac_counter_num`).each(function(){let t=e(this),n=t.data(`target`);if(!n)return;let r=parseInt(n),i=null,a=e=>{i||=e;let n=Math.min((e-i)/2e3,1),o=1-(1-n)**4,s=Math.floor(o*r);t.text(s.toLocaleString()),n<1&&requestAnimationFrame(a)};requestAnimationFrame(a)})},a=()=>{let r=new IntersectionObserver(t=>{t.forEach(t=>{t.isIntersecting&&e(t.target).addClass(`visible`)})},{threshold:.1});e(`.wi_fadeUp`).each(function(){r.observe(this)});let a=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&(i(),a.disconnect())})},{threshold:.5}),o=document.querySelector(`.ac_counter_band`);o&&a.observe(o),e(`#ac_compartir`).on(`click`,function(){let e=window.location.origin;navigator.share?navigator.share({title:t,text:`🔒 Acceso interno de ${t} - Plataforma de Ventas.`,url:e}).catch(()=>{}):n(e,this,`¡Link copiado!`)}),window.wiInitTips&&window.wiInitTips(),console.log(`🔒 ${t} Acerca actualizado`),window.__WIREADY__=!0},o=()=>{e(`#ac_compartir`).off(`click`)};export{o as cleanup,a as init,r as render};