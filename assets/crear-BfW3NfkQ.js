import{C as e,f as t,h as n,i as r}from"./widev-BkR2Na_W.js";import{A as i,N as a,O as o,S as s,T as c,g as l,m as u,w as d,x as f,y as p}from"./firebase-BXqel3Di.js";import{n as m}from"./firebase-Cc-Gk9nK.js";var h=null,g=t(`misProyectosLinkwii`)||[],_=g.length>0?g[0].slug:null,v=[{hex:`#FFFFFF`,name:`auto`},{hex:`#FFDA34`,name:`Oro`},{hex:`#3cd741`,name:`Success`},{hex:`#ffa726`,name:`Warning`},{hex:`#00a8e6`,name:`Info`},{hex:`#0EBEFF`,name:`Cielo`},{hex:`#FF5C69`,name:`Dulce`},{hex:`#29C72E`,name:`Paz`},{hex:`#7000FF`,name:`Mora`},{hex:`#21273B`,name:`Futuro`},{hex:`#dddddd`,name:`Offline`}],y=[``,`fas fa-link`,`fab fa-instagram`,`fab fa-tiktok`,`fab fa-youtube`,`fab fa-whatsapp`,`fab fa-facebook`,`fab fa-spotify`,`fab fa-x-twitter`,`fab fa-linkedin`,`fab fa-telegram`,`fas fa-globe`,`fas fa-store`,`fas fa-envelope`,`fas fa-phone`],b=e=>e?e.includes(`instagram.com`)?`fab fa-instagram`:e.includes(`tiktok.com`)?`fab fa-tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`fab fa-youtube`:e.includes(`whatsapp.com`)||e.includes(`wa.me`)?`fab fa-whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`fab fa-facebook`:e.includes(`spotify.com`)?`fab fa-spotify`:e.includes(`twitter.com`)||e.includes(`x.com`)?`fab fa-x-twitter`:e.includes(`linkedin.com`)?`fab fa-linkedin`:e.includes(`t.me`)||e.includes(`telegram`)?`fab fa-telegram`:e.includes(`pinterest.com`)?`fab fa-pinterest`:`fas fa-link`:``,x=async()=>t(`wiSmile`)?`
  ...
  `).join(``)}},C=()=>{let t=document.getElementById(`cr_col_center`);if(!t)return;let n=g.find(e=>e.slug===_);if(!n){t.innerHTML=`
    ...
    `;return}let o=n.color||`#FFFFFF`,s=v.map(e=>`
    <div class="cr_swatch ${o===e.hex?`selected`:``}"
         style="background:${e.hex};"
         data-color="${e.hex}"
         title="${e.name}"
         onclick="window.crElegirColor(this)"></div>
  `).join(``),c=n.links||[],l=c.map((e,t)=>`
        ...
        <div style="display:flex; flex-direction:column; gap:1.5vh;">
          ...
          <div class="cr_field">
            <div class="cr_swatches">
              ${s}
              <input type="hidden" id="edit_color" value="${o}">
            </div>
          </div>
        </div>
      </div>
  ...
  `,document.getElementById(`btn_update_info`)?.addEventListener(`click`,async t=>{let o=t.currentTarget;e(o,!0,`Guardando`);try{await d(i(m,`linkwiis`,n.slug),{desc:document.getElementById(`edit_desc`).value,logo:document.getElementById(`edit_logo`).value,color:document.getElementById(`edit_color`).value,actualizado:a()}),r(`Apariencia guardada ✨`,`success`)}catch{r(`Error al guardar`)}e(o,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_add_link_arr`)?.addEventListener(`click`,async()=>{let e=[...n.links||[],{titulo:``,url:``,icon:``}];await d(i(m,`linkwiis`,n.slug),{links:e})}),document.querySelectorAll(`.btn_rm_link`).forEach(e=>{e.addEventListener(`click`,async e=>{if(!confirm(`¿Estás seguro de eliminar este enlace? Esta acción no se puede deshacer.`))return;let t=parseInt(e.currentTarget.dataset.index),a=[...n.links||[]];a.splice(t,1);try{await d(i(m,`linkwiis`,n.slug),{links:a}),r(`Enlace eliminado ✨`,`success`)}catch{r(`Error al eliminar`)}})}),document.getElementById(`btn_save_links`)?.addEventListener(`click`,async t=>{let o=t.currentTarget;e(o,!0,`Guardando`);let s=n.links.filter(e=>e.titulo.trim()||e.url.trim());s.push({titulo:``,url:``,icon:``});try{await d(i(m,`linkwiis`,n.slug),{links:s,actualizado:a()}),r(`Enlaces guardados ✨`,`success`)}catch{r(`Error al guardar enlaces`)}e(o,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_del_main`)?.addEventListener(`click`,async()=>{if(confirm(`¿Eliminar definitivamente el proyecto /${n.slug}? Esta acción no se puede deshacer.`))try{await u(i(m,`linkwiis`,n.slug)),_=null,r(`Proyecto eliminado ✨`,`success`)}catch{r(`Error al eliminar`)}})},w={instagram:`linear-gradient(45deg, #f58529, #dd2a7b, #8134af, #515bd4)`,tiktok:`#000`,youtube:`#ff0000`,whatsapp:`#25D366`,facebook:`#1877F2`,twitter:`#000`,linkedin:`#0077b5`,telegram:`#0088cc`,onlyfans:`#00aff0`,pinterest:`#E60023`,web:`#00a8e6`,link:`#FFDA34`},T=e=>e?e.includes(`instagram.com`)?`instagram`:e.includes(`tiktok.com`)?`tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`youtube`:/whatsapp|wa\.me/.test(e)?`whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`facebook`:/twitter\.com|x\.com/.test(e)?`twitter`:e.includes(`linkedin.com`)?`linkedin`:/t\.me|telegram/.test(e)?`telegram`:e.includes(`onlyfans.com`)?`onlyfans`:e.includes(`pinterest.com`)?`pinterest`:/\.(com|net|org|co|info|es|app|io|me)/.test(e)?`web`:`link`:`link`,E=()=>{let e=document.getElementById(`cr_phone_render`);if(!e)return;let t=g.find(e=>e.slug===_);if(!t){e.innerHTML=`<div class="ad_empty" style="margin-top:40%; font-size:0.75rem; color:var(--tx3);"><i class="fas fa-mobile-alt" style="font-size:2rem; display:block; margin-bottom:1vh; opacity:0.3;"></i>En vivo</div>`;return}let n=t.color||`#FFFFFF`,r=(t.links||[]).filter(e=>e.titulo).map(e=>{let t=e.icon||b(e.url),r=n;(!r||r.toLowerCase()===`#ffffff`)&&(r=w[T(e.url)]);let i=r&&r.includes(`gradient`);return`
