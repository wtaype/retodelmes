import{t as U,$ as a,v as G,x as N,M as k,N as c,y as F,d as S,a as u,j as q,i as w,l as g,b as P,c as y,z as A,g as J,q as V,e as $,h as L}from"./widev-Bw9TxyzX.js";let f=null;U(L,async s=>{if(!s)return window.location.href="/";f=s;try{const e=A("wiSmile");if(e)return D(e);const o=(await P(V(y(u,"smiles"),$("usuario","==",s.displayName)))).docs[0].data();g("wiSmile",o,450),D(o)}catch(e){console.error(e)}});a(document).on("click",".bt_salir",async()=>{await G(L),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(e=>localStorage.removeItem(e))}});a(document).on("click",".tab-btn",function(){const s=a(this).data("tab");N(this,"active"),N("#"+s+"-tab","active")});a(document).on("click",".bt_cargar",()=>{const s=/^(im\d+|ki\d+|remote:im\d+|dirty:im\d+|dirty:ki\d+)$/;Object.keys(localStorage).filter(e=>s.test(e)).forEach(e=>localStorage.removeItem(e)),k("Actualizado"),setTimeout(()=>location.reload(),800)});let T="2025-09",p=1;const I=5;let d=[],v=[];function D(s){console.log(s.nombre),k("Bienvenido "+s.nombre+"!"),a(".app").html(`
    ...
    `),z()}async function z(s){try{await Promise.all([j(),M(),x()]),_(),C()}catch(e){console.error("Error inicializando dashboard:",e),c("Error cargando datos del dashboard","error")}}async function j(){try{const s=A("empleadosSmile");s&&(v=s,h());const e=V(y(u,"smiles"),$("participa","==","si"));v=(await P(e)).docs.map(o=>({id:o.id,...o.data()})),g("empleadosSmile",v,300),await O(),h()}catch(s){console.error("Error cargando empleados:",s),a("#workersGrid").html(`
        ...
        `}).join("");a("#workersGrid").html(s)}async function M(){try{d=(await P(y(u,"registrosdb"))).docs.map(e=>({id:e.id,...e.data()})),d.sort((e,i)=>{const o=new Date(e.fechaTour||"1970-01-01");return new Date(i.fechaTour||"1970-01-01")-o}),b()}catch(s){console.error("Error cargando ventas:",s),a("#salesTableBody").html(`
            <tr><td colspan="9" class="error-cell">
                <i class="fas fa-exclamation-triangle"></i> Error cargando ventas
            </td></tr>
        `)}}function b(s="",e=!1){let i=[...d];if(i=i.filter(t=>t.fechaTour&&t.fechaTour.startsWith(T)),s&&(i=i.filter(t=>t.vendedor===s)),e){const t=new Date().toISOString().split("T")[0];i=i.filter(m=>m.fechaTour===t)}const o=Math.ceil(i.length/I),n=(p-1)*I,l=i.slice(n,n+I).map(t=>{const H=t.vendedor===f?.displayName?`<button class="btn-view" onclick="verDetalleVenta('${t.id}')" title="Ver detalles"><i class="fas fa-eye"></i></button>
        ...
        `}).join("");a("#salesTableBody").html(l||'<tr><td colspan="9" class="empty-cell"><i class="fas fa-inbox"></i> No hay ventas para mostrar</td></tr>'),W(o)}function W(s){if(s<=1){a("#paginationContainer").html("");return}let e='<div class="pagination">';p>1&&(e+=`<button class="page-btn" onclick="cambiarPagina(${p-1})">
        ...
        </button>`),e+="</div>",a("#paginationContainer").html(e)}async function x(){try{const s=A("ultimoGanador");s&&R(s);const i=`${Q(T).replace("-","")}`,o=await J(S(u,"ganadores",i));if(o.exists()){const n=o.data();g("ultimoGanador",n,3600),R(n)}else a("#lastWinner").html(`
...
Esta acción es IRREVERSIBLE.`)||Y(s)};function B(s,e=!1){E(),a("#tipoTour").val(s.tipoTour),a("#registroEn").val(s.registroEn),a("#nombreCliente").val(s.nombreCliente),a("#numeroHabitacion").val(s.numeroHabitacion||""),a("#tipoDocumento").val(s.tipoDocumento||"dni"),a("#numeroDocumento").val(s.numeroDocumento||""),a("#cantidadPax").val(s.cantidadPax||1),a("#precioUnitario").val(s.precioUnitario||0),a("#metodoPago").val(s.metodoPago||""),a("#importeTotal").val(s.importeTotal||0),a("#cobroProveedor").val(s.cobroProveedor||0),a("#horaSalida").val(s.horaSalida),a("#Operador").val(s.Operador),a("#Comentario").val(s.Comentario),a("#fechaTour").val(s.fechaTour),a("#estadoPago").val(s.estadoPago||"pagado"),a("#vistaPreviaLaPuntos").text(s.puntos||0),e?(a("#formularioVenta input, #formularioVenta select").prop("disabled",!0),a(".btn-save").prop("disabled",!0).html('<i class="fas fa-eye"></i> Solo Vista'),a("#formularioVenta").addClass("view-only"),a(".btn-clear-view").length===0&&a(".form-actions").prepend(`
            ...
            `))}function E(){a("#formularioVenta input, #formularioVenta select").prop("disabled",!1),a(".btn-save").prop("disabled",!1),a("#formularioVenta").removeClass("view-only edit-mode"),a(".btn-save").html('<i class="fas fa-save"></i> Guardar Venta').removeAttr("data-edit-id"),a(".btn-clear-view, .btn-cancel-edit").remove(),a("#formularioVenta")[0].reset(),a("#cantidadPax").val(1),a("#vistaPreviaLaPuntos").text("0")}async function Y(s){try{c("Eliminando venta...","info"),await F(S(u,"registrosdb",s));const e=[];for(let o=0;o<localStorage.length;o++){const n=localStorage.key(o);if(n&&n.startsWith("vendedor_"))try{const r=JSON.parse(localStorage.getItem(n));r&&r.idVenta===s&&e.push(n)}catch{}}e.forEach(o=>localStorage.removeItem(o)),d=d.filter(o=>o.id!==s),await O(),h(),b(),C(),a(".btn-save").attr("data-edit-id")===s&&E(),c("¡Venta eliminada exitosamente!","success")}catch(e){console.error("Error eliminando venta:",e),c("Error al eliminar la venta. Inténtalo nuevamente.","error")}}a(document).on("click",".btn-clear-view",function(){E(),c("Vista limpiada","info")});a(document).on("click",".btn-cancel-edit",function(){E(),c("Edición cancelada","info")});function Q(s){const[e,i]=s.split("-"),o=new Date(parseInt(e),parseInt(i)-2);return`${o.getFullYear()}-${String(o.getMonth()+1).padStart(2,"0")}`}function X(){return`
