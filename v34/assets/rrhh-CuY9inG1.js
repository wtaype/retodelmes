import{r as e}from"./vendor-CZ6bxb2j.js";import{f as t,h as n,i as r,m as i,n as a,t as o}from"./widev-BkR2Na_W.js";import"./index-4xtKSgn6.js";import{M as s,O as c,_ as l,k as u,w as d}from"./firebase-COMEVlQu.js";import{n as f}from"./firebase-srJqeJFD.js";var p=[],m=`todos`,h=``,g=null,_=!1,v=`rrhhUsuarios`,y=30,b=e=>((e.nombre||``)+` `+(e.apellidos||``)||e.usuario||`?`).trim().split(/\s+/).slice(0,2).map(e=>(e[0]||``).toUpperCase()).join(``),x=(e,t=40)=>{if(e.imagen)return`<div class="rrhh_avatar" style="width:${t}px;height:${t}px"><img src="${e.imagen}" alt="${o(e.nombre||e.usuario||`?`)}" loading="lazy"/></div>`;let n=b(e);return`<div class="rrhh_avatar rrhh_avatar_initials" data-rol="${e.rol||`smile`}" style="width:${t}px;height:${t}px;font-size:${Math.round(t*.36)}px">${n}</div>`},S=e=>{let t=(e||`smile`).toLowerCase();return`<span class="rrhh_rol_badge rrhh_rol_${t}">${o(t)}</span>`},C=(e,t)=>{let n=t===`activo`;return`
...
`,D=()=>{let t=p.length,n=p.filter(e=>e.participa===`si`).length,r=p.filter(e=>e.estado===`pendiente`).length,i=p.filter(e=>e.participa!==`si`).length;e(`#stat_total`).text(t),e(`#stat_activos`).text(n),e(`#stat_pendientes`).text(r),e(`#stat_inactivos`).text(i)},O=()=>{D();let t=T();if(t.sort((e,t)=>{let n=e.participa===`si`?0:1,r=t.participa===`si`?0:1;return n===r?(e.nombre||e.usuario||``).localeCompare(t.nombre||t.usuario||``,`es`):n-r}),!t.length){let t=h.toLowerCase().trim();e(`#rrhh_tbody`).html(`
    ...
    `}).join(``);e(`#rrhh_tbody`).html(n)},k=n=>{let r=p.find(e=>e.id===n);if(!r)return;g=n;let i=a((r.nombre||``)+` `+(r.apellidos||``)).trim()||r.usuario||`—`;e(`#rrhh_panel_avatar`).html(x(r,52)),e(`#rrhh_panel_name`).text(i),e(`#rrhh_panel_user`).text(`@`+(r.usuario||`—`)),e(`#edit_uid`).val(n),e(`#edit_nombre`).val(r.nombre||``),e(`#edit_apellidos`).val(r.apellidos||``),e(`#edit_usuario`).val(r.usuario||``),e(`#edit_email`).val(r.email||``),e(`#edit_descripcion`).val(r.descripcion||``),e(`#edit_imagen`).val(r.imagen||``),e(`#edit_rol`).val(r.rol||`smile`),e(`#edit_estado`).val(r.estado||`activo`),e(`#edit_participa`).prop(`checked`,r.participa===`si`),e(`#edit_banco`).val(r.banco||``),e(`#edit_numeroCuenta`).val(r.numeroCuenta||``),e(`#edit_titularCuenta`).val(r.titularCuenta||``),(t(`wiSmile`)?.rol||`smile`)===`gestor`?e(`#edit_rol option[value="admin"]`).hide():e(`#edit_rol option[value="admin"]`).show(),e(`#rrhh_panel`).addClass(`open`).attr(`aria-hidden`,`false`),e(`#rrhh_overlay`).addClass(`visible`),e(`body`).addClass(`rrhh_no_scroll`)},A=()=>{g=null,e(`#rrhh_panel`).removeClass(`open`).attr(`aria-hidden`,`true`),e(`#rrhh_overlay`).removeClass(`visible`),e(`body`).removeClass(`rrhh_no_scroll`)},j=async e=>{let t=p.findIndex(t=>t.id===e);if(t===-1)return;let a=p[t].participa===`si`?`no`:`si`,s=o(p[t].nombre||p[t].usuario||e);try{await d(u(f,`smiles`,e),{participa:a}),p[t].participa=a,i(v),n(v,p,y),O(),r(`${s} ahora ${a===`si`?`participa ✅`:`no participa ❌`}`,a===`si`?`success`:`warning`)}catch(e){console.error(`[RRHH] toggleParticipa:`,e),r(`Error al actualizar participación`,`error`),O()}},M=async e=>{let t=p.findIndex(t=>t.id===e);if(t===-1)return;let a=p[t].estado===`activo`?`inactivo`:`activo`,s=o(p[t].nombre||p[t].usuario||e);try{await d(u(f,`smiles`,e),{estado:a}),p[t].estado=a,i(v),n(v,p,y),O(),r(`${s} ahora está ${a===`activo`?`Activo ✅`:`Inactivo ❌`}`,a===`activo`?`success`:`warning`)}catch(e){console.error(`[RRHH] toggleEstado:`,e),r(`Error al actualizar estado`,`error`),O()}},N=async()=>{if(_||!g)return;_=!0;let t=e(`#rrhh_btn_save`);t.addClass(`loading`).prop(`disabled`,!0),e(`#rrhh_header_card`).addClass(`smw_loading`);let a={nombre:e(`#edit_nombre`).val().trim(),apellidos:e(`#edit_apellidos`).val().trim(),usuario:e(`#edit_usuario`).val().trim(),email:e(`#edit_email`).val().trim(),descripcion:e(`#edit_descripcion`).val().trim(),imagen:e(`#edit_imagen`).val().trim(),rol:e(`#edit_rol`).val(),estado:e(`#edit_estado`).val(),participa:e(`#edit_participa`).is(`:checked`)?`si`:`no`,banco:e(`#edit_banco`).val(),numeroCuenta:e(`#edit_numeroCuenta`).val().trim(),titularCuenta:e(`#edit_titularCuenta`).val().trim(),updatedAt:s()};Object.keys(a).forEach(e=>{a[e]===``&&delete a[e]});try{await d(u(f,`smiles`,g),a);let e=p.findIndex(e=>e.id===g);e!==-1&&Object.assign(p[e],a),i(v),n(v,p,y),O(),A(),r(`Colaborador actualizado ✅`,`success`)}catch(e){console.error(`[RRHH] saveEdit:`,e),r(`Error al guardar cambios`,`error`)}finally{_=!1,t.removeClass(`loading`).prop(`disabled`,!1),e(`#rrhh_header_card`).removeClass(`smw_loading`)}},P=async e=>{let t=p.findIndex(t=>t.id===e);if(t===-1)return;let a=o(p[t].nombre||p[t].usuario||e);try{await d(u(f,`smiles`,e),{estado:`activo`,participa:`si`}),p[t].estado=`activo`,p[t].participa=`si`,i(v),n(v,p,y),O(),r(`${a} aprobado como colaborador ✅`,`success`)}catch(e){console.error(`[RRHH] aprobar:`,e),r(`Error al aprobar solicitud`,`error`)}},F=async e=>{let t=p.findIndex(t=>t.id===e);if(t===-1)return;let a=o(p[t].nombre||p[t].usuario||e);try{await d(u(f,`smiles`,e),{estado:`inactivo`,participa:`no`}),p[t].estado=`inactivo`,p[t].participa=`no`,i(v),n(v,p,y),O(),r(`Solicitud de ${a} rechazada`,`warning`)}catch(e){console.error(`[RRHH] rechazar:`,e),r(`Error al rechazar solicitud`,`error`)}},I=async(i=!1)=>{if(!i){let e=t(v);if(e){p=e,O();return}}e(`#rrhh_tbody`).html(`
    <tr>
      <td colspan="8" class="rrhh_loading_cell">
        <div class="rrhh_spinner"><i class="fas fa-circle-notch fa-spin"></i> Cargando colaboradores…</div>
      </td>
    </tr>
  `);try{p=(await l(c(f,`smiles`))).docs.map(e=>({id:e.id,...e.data()})),p.sort((e,t)=>{let n=e.participa===`si`?0:1,r=t.participa===`si`?0:1;return n===r?(e.nombre||e.usuario||``).localeCompare(t.nombre||t.usuario||``,`es`):n-r}),n(v,p,y),O()}catch(t){console.error(`[RRHH] loadUsuarios:`,t),e(`#rrhh_tbody`).html(`
      <tr>
        <td colspan="8">
