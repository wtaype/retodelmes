import{r as e}from"./vendor-CZ6bxb2j.js";import{l as t}from"./wii-BvK6d7hI.js";import{S as n,_ as r,b as i,d as a,i as o,r as s,w as c}from"./widev-qaLPAUWY.js";import{i as l}from"./index-DgKayDHB.js";import{M as u,S as d,g as f,k as p,w as m}from"./firebase-COMEVlQu.js";import{n as h}from"./firebase-srJqeJFD.js";import{a as g,b as _,o as v,t as y,u as b}from"./devblog-CnKgvXbx.js";var x=e=>e.trim().toLowerCase().normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).replace(/\b(el|la|los|las|de|del|en|un|una|y|a|con|por|para|que|es|se)\b/g,` `).replace(/[^a-z0-9\s]/g,``).replace(/\s+/g,`_`).replace(/_{2,}/g,`_`).replace(/^_|_$/g,``).slice(0,50),S=e=>{let t=e.replace(/<[^>]*>/g,` `).split(/\s+/).filter(Boolean);return{words:t.length,min:Math.max(1,Math.ceil(t.length/200))}},C=()=>i.params()?.edit||new URLSearchParams(location.search).get(`edit`)||null,w=()=>{let e=r.user?.usuario?r.user:a(`wiSmile`)||{};if(!e.email)return`<div class="nu_err dpvc"><i class="fas fa-lock"></i><h2>Acceso restringido</h2><p>Inicia sesión para crear historias</p></div>`;let n=C();return`
  <div class="nu_wrap">
    <div class="nu_head">
      ...
      <div class="nu_head_right">
        ${n?`<a href="/${n}" class="nu_btn_outline" ${c(`Ver post`)}><i class="fas fa-eye"></i> Ver</a>`:`<button type="button" id="nu_preview_pg" class="nu_btn_outline" ${c(`Preview`)}><i class="fas fa-eye"></i> Preview</button>`}
      ...
      <div class="nu_left">
        <div class="nu_card">
          ...
          <div class="nu_slug_box">
            ...
            ${n?``:`<button type="button" id="nu_slug_reset" class="nu_slug_btn" ${c(`Regenerar`)}><i class="fas fa-rotate"></i></button>`}
          </div>
...
---
`,`Separador`]],[[`fa-code`,"`código`",`Código`],[`fa-image`,`![desc](url)`,`Imagen`],[`fa-link`,`[texto](url)`,`Enlace`]]].map((e,t)=>`<div style="display:flex;gap:0.4vh${t<3?`;border-right:1px solid var(--brd);padding-right:0.6vh;margin-right:0.2vh`:``}">${e.map(([e,t,n])=>`<button type="button" class="nu_tool" data-tag='${t}' ${c(n)}><i class="fas ${e}"></i></button>`).join(``)}</div>`).join(``)}</div>
      ...
      <div class="nu_right">
        ...
        <div class="nu_card">
          ...
          <div style="display:flex; flex-direction:column; gap:1vh; margin-bottom: 1.5vh;">
            ...
            <div id="nu_img_prev" class="nu_img_prev dpn"><img id="nu_img_el" src="" alt=""/><button type="button" id="nu_img_clear" class="nu_img_clear" ${c(`Quitar`)}><i class="fas fa-xmark"></i></button></div>
          </div>
          <div style="display:flex; flex-direction:column; gap:1vh;">
            ...
            <div id="nu_img_top_prev" class="nu_img_prev dpn"><img id="nu_img_top_el" src="" alt=""/><button type="button" id="nu_img_top_clear" class="nu_img_clear" ${c(`Quitar`)}><i class="fas fa-xmark"></i></button></div>
          </div>
        </div>
...
`)},k=w?`wi_draft_edit_`+t:`wi_draft_new`,A=()=>{let t={titulo:e(`#nu_titulo`).val(),slug:e(`#nu_slug_inp`).val(),resumen:e(`#nu_resumen`).val(),keywords:e(`#nu_keywords`).val(),cat:e(`#nu_cat_inp`).val(),img:e(`#nu_img`).val(),imgTop:e(`#nu_img_top`).val(),content:e(`#nu_contenido`).val(),tags:i};localStorage.setItem(k,JSON.stringify(t))};if(e(`#nu_form`).on(`input`,`input, textarea`,()=>{clearTimeout(window.wiDraftTimer),window.wiDraftTimer=setTimeout(A,1e3)}),(()=>{try{let t=new Set,n=new Set;for(let e of Object.keys(localStorage).filter(e=>e.startsWith(`wi_blogs`)||e.startsWith(`wi_post_`))){let r=a(e);(Array.isArray(r)?r:r?[r]:[]).forEach(e=>{e.categoria&&t.add(e.categoria),e.tags&&Array.isArray(e.tags)&&e.tags.forEach(e=>n.add(e))})}t.size>0&&e(`#nu_cat_sug`).html(Array.from(t).slice(0,8).map(e=>`<span class="nu_sug_chip cat_sug">${e}</span>`).join(``)),n.size>0&&e(`#nu_tag_sug`).html(Array.from(n).slice(0,12).map(e=>`<span class="nu_sug_chip tag_sug">#${e}</span>`).join(``))}catch{console.warn(`No se pudieron cargar sugerencias`)}})(),w)try{let n=await b(t,!0);if(!n?.data){o(`Post no encontrado`,`error`);return}let r=n.data;e(`#nu_titulo`).val(r.titulo),e(`#nu_slug_inp`).val(r.slug||r.id),e(`#nu_resumen`).val(r.resumen||``).trigger(`input`),e(`#nu_keywords`).val(r.keywords||``),e(`#nu_contenido`).val(r.contenidoMd||r.contenido||``),e(`#nu_img`).val(r.imagen||``),e(`#nu_img_top`).val(r.imagenTop||``),e(`#nu_activo`).prop(`checked`,r.activo!==!1),e(`#nu_pin`).prop(`checked`,!!r.pin),e(`#nu_cat_inp`).val(r.categoria||``),i=Array.isArray(r.tags)?[...r.tags]:[],D(),r.imagen&&(e(`#nu_img_el`).attr(`src`,r.imagen),e(`#nu_img_prev`).removeClass(`dpn`)),r.imagenTop&&(e(`#nu_img_top_el`).attr(`src`,r.imagenTop),e(`#nu_img_top_prev`).removeClass(`dpn`)),e(`#nu_resumen_cnt`).text((r.resumen||``).length),E()}catch(e){console.error(`edit load:`,e),o(`Error cargando post`,`error`)}else try{let t=JSON.parse(localStorage.getItem(k));t&&(e(`#nu_titulo`).val(t.titulo),e(`#nu_slug_inp`).val(t.slug),e(`#nu_resumen`).val(t.resumen),e(`#nu_keywords`).val(t.keywords||``),e(`#nu_cat_inp`).val(t.cat),e(`#nu_img`).val(t.img).trigger(`input`),e(`#nu_img_top`).val(t.imgTop).trigger(`input`),e(`#nu_contenido`).val(t.content),t.tags&&(i=t.tags,D()),E())}catch{}e(`#nu_form`).on(`keydown`,e=>{e.key===`Enter`&&e.target.tagName!==`TEXTAREA`&&e.preventDefault()}),w||(e(`#nu_slug_inp`).on(`input`,function(){e(this).val(e(this).val().replace(/[^a-z0-9_]/gi,e=>e===` `?`_`:``).toLowerCase().replace(/_{2,}/g,`_`)),e(`#nu_slug_status`).html(`<i class="fas fa-pen"></i> Escribiendo...`).removeClass(`ok err`)}).on(`change`,async function(){let t=e(this).val(),n=e(`#nu_slug_status`);if(!t)return n.html(``).removeClass(`ok err`);if(t.length<3)return n.html(`<i class="fas fa-exclamation"></i> Muy corto`).addClass(`err`).removeClass(`ok`);n.html(`<i class="fas fa-spinner fa-spin"></i>`).removeClass(`ok err`),(await f(p(h,`blog`,t)).catch(()=>null))?.exists()?n.html(`<i class="fas fa-xmark"></i> Ya existe`).addClass(`err`).removeClass(`ok`):n.html(`<i class="fas fa-check"></i> OK`).addClass(`ok`).removeClass(`err`)}),e(`#nu_slug_reset`).on(`click`,()=>{e(`#nu_slug_inp`).val(T()).trigger(`input`).trigger(`change`)})),e(`#nu_resumen`).on(`input`,function(){e(`#nu_resumen_cnt`).text(e(this).val().length)}),e(`#nu_img`).on(`input`,function(){clearTimeout(c),c=setTimeout(()=>{let t=e(this).val().trim();if(!t)return e(`#nu_img_prev`).addClass(`dpn`);e(`#nu_img_el`).attr(`src`,t).off(`load error`).on(`load`,()=>e(`#nu_img_prev`).removeClass(`dpn`)).on(`error`,()=>e(`#nu_img_prev`).addClass(`dpn`))},600)}),e(`#nu_img_clear`).on(`click`,()=>{e(`#nu_img`).val(``).trigger(`input`),e(`#nu_img_prev`).addClass(`dpn`)});let j;e(`#nu_img_top`).on(`input`,function(){clearTimeout(j),j=setTimeout(()=>{let t=e(this).val().trim();if(!t)return e(`#nu_img_top_prev`).addClass(`dpn`);e(`#nu_img_top_el`).attr(`src`,t).off(`load error`).on(`load`,()=>e(`#nu_img_top_prev`).removeClass(`dpn`)).on(`error`,()=>e(`#nu_img_top_prev`).addClass(`dpn`))},600)}),e(`#nu_img_top_clear`).on(`click`,()=>{e(`#nu_img_top`).val(``).trigger(`input`),e(`#nu_img_top_prev`).addClass(`dpn`)}),e(`#nu_contenido`).on(`input`,E),e(`#nu_tags_inp`).on(`keydown`,function(t){t.key!==`Enter`&&t.key!==`,`||(t.preventDefault(),e(this).val().toLowerCase().split(`,`).map(e=>e.trim().replace(/\s+/g,`_`)).filter(Boolean).forEach(e=>{!i.includes(e)&&i.length<8&&i.push(e)}),D(),e(this).val(``))}),e(document).on(`click.nuevo`,`.nu_tool`,function(){let t=e(this).data(`tag`),n=e(`#nu_contenido`),r=n[0],i=r.selectionStart,a=r.selectionEnd,o=r.value.substring(i,a)||`texto`,s=t.replace(`texto`,o).replace(`cita`,o);n.val(r.value.substring(0,i)+s+r.value.substring(a)),r.focus(),r.selectionStart=i,r.selectionEnd=i+s.length,E(),A()}).on(`click.nuevo`,`.nu_tab`,function(){let t=e(this).data(`tab`);e(`.nu_tab`).removeClass(`active`),e(this).addClass(`active`),t===`prev`?(e(`#nu_prev_html`).html(_(O(e(`#nu_contenido`).val()))).removeClass(`dpn`),e(`#nu_contenido`).addClass(`dpn`)):(e(`#nu_contenido`).removeClass(`dpn`),e(`#nu_prev_html`).addClass(`dpn`))}).on(`click.nuevo`,`.nu_tag_rm`,function(){i.splice(+e(this).data(`i`),1),D()}).on(`click.nuevo`,`.cat_sug`,function(){e(`#nu_cat_inp`).val(e(this).text())}).on(`click.nuevo`,`.tag_sug`,function(){let t=e(this).text().replace(`#`,``);t&&!i.includes(t)&&i.length<8&&(i.push(t),D())}),e(`#nu_form`).on(`submit`,async function(c){c.preventDefault();let b=e(`#nu_submit,.nu_btn_full`),x=r.user?.usuario?r.user:a(`wiSmile`)||{},C=e(`#nu_cat_inp`).val().trim();C&&=C.charAt(0).toUpperCase()+C.slice(1).toLowerCase();let[T,E,D,A,j,M,N,P]=[e(`#nu_titulo`).val().trim(),e(`#nu_resumen`).val().trim(),e(`#nu_keywords`).val().trim(),C,e(`#nu_img`).val().trim(),e(`#nu_img_top`).val().trim(),e(`#nu_contenido`).val().trim(),e(`#nu_slug_inp`).val().trim()],F=_(O(N));if(!T||!E||!A||!j||!N)return o(`Completa los campos obligatorios`,`warning`);if(N.length<10)return o(`Contenido muy corto`,`warning`);if(!P||P.length<3)return o(`Slug inválido`,`warning`);if(!w&&e(`#nu_slug_status`).hasClass(`err`))return o(`Slug no disponible`,`error`);n(b,!0,w?`Guardando...`:`Publicando...`);try{if(`${S(F).min}`,w)await m(p(h,y,t),{activo:e(`#nu_activo`).is(`:checked`),pin:e(`#nu_pin`).is(`:checked`),titulo:T,resumen:E,keywords:D,categoria:A,contenido:F,contenidoMd:N,imagen:j,imagenTop:M,imagenAlt:T,tags:i,tiempoLectura:`${S(F).min} min`,actualizado:u()}),v(t),g(),s(`¡Historia actualizada! 🐾✨`,`success`),setTimeout(()=>l(()=>import(`./index-DgKayDHB.js`).then(e=>e.r).then(e=>e.rutas.navigate(`/${t}`)),[]),1200);else{if((await f(p(h,`blog`,P))).exists())return n(b,!1),o(`Slug existente`,`warning`);await d(p(h,y,P),{id:P,slug:P,activo:e(`#nu_activo`).is(`:checked`),pin:e(`#nu_pin`).is(`:checked`),usuario:x.usuario,email:x.email,autor:x.nombre||x.usuario,titulo:T,resumen:E,keywords:D,categoria:A,contenido:F,contenidoMd:N,imagen:j,imagenTop:M,imagenAlt:T,tags:i,vistas:0,likes:0,tiempoLectura:`${S(F).min} min`,creado:u(),actualizado:u()}),g(),localStorage.removeItem(k),s(`¡Historia publicada! 🐾✨`,`success`),setTimeout(()=>l(()=>import(`./index-DgKayDHB.js`).then(e=>e.r).then(e=>e.rutas.navigate(`/${P}`)),[]),1200)}}catch(e){console.error(`nuevo:`,e),o(w?`Error al guardar`:`Error al publicar`,`error`),n(b,!1)}})},E=()=>{e(`#nu_form,#nu_slug_inp,#nu_titulo,#nu_resumen,#nu_img,#nu_contenido`).off(),e(document).off(`.nuevo`)};export{E as cleanup,T as init,w as render};