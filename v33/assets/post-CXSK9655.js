import{r as e}from"./vendor-CZ6bxb2j.js";import{i as t,t as n}from"./wii-BvK6d7hI.js";import{E as r,_ as i,d as a,g as o,h as s,i as c,w as l,x as u}from"./widev-qaLPAUWY.js";import{i as d}from"./index-DgKayDHB.js";import{c as f,f as p,m,n as h,o as g,p as _,r as v,s as y,u as b,v as x,y as S}from"./devblog-CnKgvXbx.js";e(`#wiad_styles`).length||e(`<style id="wiad_styles">`).text(`
...
`,w=e=>`<a href="/${e.slug||e.id}" class="po_rel_card" ${l(e.resumen||e.titulo)}><div class="po_rel_img"><img src="${e.imagen}" alt="${e.imagenAlt||e.titulo}" loading="lazy"/></div><div class="po_rel_info"><span class="po_rel_cat"><i class="fas fa-paw"></i> ${e.categoria}</span><strong>${e.titulo}</strong><span class="po_rel_meta"><i class="fas fa-calendar"></i> ${x(e.actualizado||e.creado)} · <i class="fas fa-eye"></i> ${e.vistas||0} · <i class="fas fa-heart" style="color:#fe0149"></i> ${e.likes||0}</span></div></a>`,T=(e,t,n=`po_fade po_visible`)=>e||t?`
    <div class="po_pn_box ${n}" style="--d:.4s">
      ${e?`<a href="/${e.slug||e.id}" class="po_pn_card pn_prev" ${l(`Anterior`)}><div class="po_pn_img"><img src="${e.imagen}" loading="lazy"/></div><div class="po_pn_info"><span class="po_pn_lb"><i class="fas fa-arrow-left"></i> Anterior</span><strong class="po_pn_tit">${e.titulo}</strong></div></a>`:`<div></div>`}
      ${t?`<a href="/${t.slug||t.id}" class="po_pn_card pn_next" ${l(`Siguiente`)}><div class="po_pn_info"><span class="po_pn_lb">Siguiente <i class="fas fa-arrow-right"></i></span><strong class="po_pn_tit">${t.titulo}</strong></div><div class="po_pn_img"><img src="${t.imagen}" loading="lazy"/></div></a>`:`<div></div>`}
    </div>`:``,E=(e,r,a,o,s)=>{let c=i.user?.usuario,u=s?`po_fade po_visible`:`po_fade`,d=r!==null&&a!==null,f=r||[],p=a||[],m=f[0]||p[0],h=f[1]||p[1],g=d?T(m,h,u):`<div id="po_nav_container"></div>`;return`
      ...
      <div class="po_col_main"><div class="po_content">
        <div class="po_hero ${u}" style="--d:0s">
          ...
          <div class="po_hero_over">
            <a href="/blog" class="po_back" ${l(`Volver`)}><i class="fas fa-arrow-left"></i> Blog</a>
            <div class="po_hero_badges">
              <span class="po_cat_badge" ${l(e.categoria)}><i class="fas fa-paw"></i> ${e.categoria}</span>
              ${e.pin?`<span class="po_dest_badge" ${l(`Destacada`)}><i class="fas fa-thumbtack"></i> Pin</span>`:``}
              <button class="po_like_btn po_like_sync ${localStorage.getItem(`wi_like_`+o)?`active`:``}" data-slug="${o}" ${l(`Me encanta`)} style="border-color:rgba(255,255,255,0.2);background:rgba(0,0,0,0.4);color:#fff;padding:.6vh 1.2vh;font-size:var(--fz_s4)"><i class="fas fa-heart"></i> <span class="po_likes_count_text">${e.likes||0}</span></button>
            </div>
          </div>
        </div>
        <header class="po_header ${u}" style="--d:.1s">
          ...
          <div class="po_meta">
            <span ${l(`Autor`)}><i class="fas fa-user-pen"></i> ${e.autor}</span>
            <span ${l(`Fecha`)}><i class="fas fa-calendar"></i> ${x(e.creado,!0)}</span>
            <span ${l(`Lectura`)}><i class="fas fa-clock"></i> ${e.tiempoLectura}</span>
            <span ${l(`Vistas`)}><i class="fas fa-eye"></i> ${(e.vistas||0)+1}</span>
            <button class="po_like_btn po_like_sync ${localStorage.getItem(`wi_like_`+o)?`active`:``}" data-slug="${o}" ${l(`Me encanta`)}><i class="fas fa-heart"></i> <span class="po_likes_count_text">${e.likes||0}</span></button>
            ${s?`<span class="po_cache_badge" ${l(`Cache ⚡`)}><i class="fas fa-bolt"></i> Local</span>`:``}
          </div>
        </header>
        ...
        <div class="po_share ${u}" style="--d:.45s"><span><i class="fas fa-share-nodes"></i> Comparte</span>
          <div class="po_share_btns">
            <button class="po_like_btn po_like_sync ${localStorage.getItem(`wi_like_`+o)?`active`:``}" data-slug="${o}" ${l(`Me encanta`)}><i class="fas fa-heart"></i> <span class="po_likes_count_text">${e.likes||0}</span></button>
            ${S(e.titulo)}<button class="po_share_btn po_copy" style="--sc:var(--mco)" ${l(`Copiar`)}><i class="fas fa-link"></i></button>
          </div>
        </div>
      </div>
    ...
    <aside class="po_sidebar">
      <div class="po_side_card ${u}" style="--d:.15s">
        ...
        ${c?`<div class="po_admin_actions" style="margin-top:.8vh"><a href="/nuevo?edit=${o}" class="po_admin_btn_edit" ${l(`Editar`)}><i class="fas fa-pen"></i> Editar</a><button id="po_refresh" class="po_admin_btn_refresh" data-slug="${o}" data-cat="${e.categoria}" ${l(`Recargar`)}><i class="fas fa-rotate"></i></button></div>`:``}
      </div>
  ...
  </div></div>`},D=!1,O=e=>{let t=e&&a(`wi_post_${e}`);if(t)return D=!0,E(t,a(`wi_mas_${t.categoria}`)||[],a(`wi_sidebar_posts`)||[],e,!0);D=!1;let n=e&&p(e);return n?`
    <div class="po_wrap"><div class="po_layout"><div class="po_col_main"><div class="po_content">
      <div class="po_hero po_fade po_visible" style="--d:0s"><img src="${n.imagenTop||n.imagen}" alt="${n.imagenAlt||n.titulo}" class="po_hero_img" loading="eager"/><div class="po_hero_over"><a href="/blog" class="po_back" ${l(`Volver`)}><i class="fas fa-arrow-left"></i> Blog</a><div class="po_hero_badges"><span class="po_cat_badge"><i class="fas fa-paw"></i> ${n.categoria}</span></div></div></div>
    ...
    </div></div><aside class="po_sidebar" style="min-height:98vh;">${`<div class="po_sk_side shimmer"></div>`.repeat(3)}</aside></div></div>`:`<div class="po_wrap"><div class="po_layout"><div class="po_col_main"><div class="po_content"><div class="po_sk_img shimmer"></div><div class="po_sk_body" style="min-height:98vh;"><div class="po_sk_cat shimmer"></div><div class="po_sk_tit shimmer"></div><div class="po_sk_meta shimmer"></div>${`<div class="po_sk_p shimmer"></div>`.repeat(5)}</div></div></div><aside class="po_sidebar" style="min-height:98vh;">${`<div class="po_sk_side shimmer"></div>`.repeat(3)}</aside></div></div>`},k=async(t,r=!1)=>{if(t){D&&!r&&(f(`po_fade`),o(),A(),j());try{let i=await b(t,r);if(!i?.data?.activo)return e(`#wimain`).html(`<div class="po_err dpvc"><i class="fas fa-paw"></i><h2>Historia no encontrada</h2><p>No existe o no está disponible 🐾</p><a href="/blog" class="po_back_btn"><i class="fas fa-arrow-left"></i> Ver historias</a></div>`);let{data:a,fromCache:c}=i;!c&&!r&&v(t),s({title:a.titulo,desc:a.resumen,keywords:a.keywords,img:a.imagenTop||a.imagen,path:`/${t}`,type:`Article`,datePublished:a.creado}),!D||r?(e(`#wimain`).html(E(a,null,null,t,c)),f(`po_fade`),o(),A(),j()):c||e(`.po_cache_badge`).remove();let l=(t,n)=>{e(`#po_nav_container`).html(T(t[0]||n[0],t[1]||n[1])),e(`#po_ultimas_container`).html(n.length?`<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-clock"></i> Últimas historias</div><div class="po_relacionados" data-showi="100">${n.map(w).join(``)}</div></div>`:``),e(`#po_rels_container`).html(t.length?`<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-heart"></i> Más historias de ${a.categoria}</div><div class="po_relacionados" data-showi="100">${t.map(w).join(``)}</div></div>`:``),o()};Promise.all([_(t,a.categoria,r),m(t,r)]).then(([e,t])=>{l(e||[],t||[])});let d=function(){this.page.url=`https://wiihope.com/${t}`,this.page.identifier=t,this.page.title=a.titulo};window.DISQUS?window.DISQUS.reset({reload:!0,config:d}):(window.disqus_config=d,u({disqus:[async()=>e(`body`).append(e(`<script>`,{src:`https://superwii.disqus.com/embed.js`,"data-timestamp":+new Date}))]})),console.log(`🐾 ${n} Post OK`),window.__WIREADY__=!0}catch(e){console.error(`[post]`,e),c(`Error al cargar`,`error`)}}};function A(){if(e(`.po_toc_box`).length)return;let t=e(`.po_contenido`).find(`h2,h3`);if(!t.length)return;let n=`<div class="po_toc_box po_fade" style="--d:.15s"><div class="po_toc_title"><i class="fas fa-list"></i> En este artículo</div><ul class="po_toc">`;t.each((t,r)=>{let i=`po_h_`+t;e(r).attr(`id`,i).css(`scroll-margin-top`,`7vh`),n+=`<li style="margin-left:${r.tagName.toLowerCase()===`h3`?`1.5vh`:`0`}"><a href="#${i}" class="po_toc_link">${e(r).text()}</a></li>`}),e(n+`</ul></div>`).insertBefore(`.po_contenido`)}function j(){e(window).off(`scroll.post_prog`).on(`scroll.post_prog`,()=>e(`#po_progress`).css(`width`,e(window).scrollTop()/Math.max(e(document).height()-e(window).height(),1)*100+`%`))}e(document).on(`click.post`,`.po_copy,.po_copy2`,()=>r(location.href,`.po_copy`,`¡Enlace copiado! 🔗`)).on(`click.post`,`.po_rel_card`,function(t){t.preventDefault(),d(()=>import(`./index-DgKayDHB.js`).then(e=>e.r).then(t=>t.rutas.navigate(e(this).attr(`href`))),[])}).on(`click.post`,`.po_like_sync`,function(){let t=e(this).data(`slug`);localStorage.getItem(`wi_like_`+t)||(localStorage.setItem(`wi_like_`+t,`1`),e(`.po_like_sync`).addClass(`active`),e(`.po_likes_count_text`).text((parseInt(e(`.po_likes_count_text`).first().text())||0)+1),h(t))}).on(`click.post`,`.po_yt_btn`,async function(){let t=e(this).data(`yt`);e(`#wi_yt_modal`).length||(e(`body`).append(`
