import{o as ct,a as m,b as S,q as z,c as N,w as T,f as tt,$ as s,n as q,l as lt,d as F,i as M,p as O,h as G,m as dt}from"./init-D9JqHDy7.js";import"./foo-D1XQkuJl.js";import{g as I,i as W,s as k,N as i,m as K,C as H,r as D}from"./widev-BIaVfPmz.js";let f=null,g=new Date(new Date().toLocaleString("en-US",{timeZone:"America/Lima"})).toISOString().slice(0,7),b="todos",d=0,P=0,v=[],x=[],u=[],L=7,p=[],_=!1,j=null;ct(tt,async t=>{if(!t)return window.location.href="/";j=t;try{const o=I("wiSmileTop");if(o)return Y(o),W(m,j);const e=(await S(z(N(m,"smiles"),T("usuario","==",t.displayName)))).docs[0].data();k("wiSmileTop",e,450),Y(e),W(m,j)}catch(o){console.error(o)}});async function Y(t){f=t,console.log(`✅ Admin: ${t.nombre}`),s(".app").html(ut());try{await at(),await V(),await pt(),await R(),await vt(),bt(),A(),C(),i("Bienvenido "+t.nombre,"success")}catch(o){console.error("Error init:",o),i("Error al inicializar","error")}}function ut(){return`
    ...
    `;t.html(o);const a=v.reduce((n,r)=>n+(parseFloat(r.importeTotal)||0),0),e=v.reduce((n,r)=>n+(parseInt(r.puntos)||0),0);Q(v,a,e),ht()}function ot(t,o=!1){const a=x.find($=>$.usuario===t.vendedor||$.id===t.vendedor),e=mt(t.fechaTour),n=c(t.precioUnitario),r=c(t.importeTotal),l=c(t.comision,r*.1),y=c(t.ganancia,r-l),h=parseInt(t.puntos)||0;function c($,E=0){const w=parseFloat($);return isNaN(w)?E:w}return o?`
