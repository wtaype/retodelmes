import{b as V,c as k,a as P,$ as t,g as ba,d as R,i as na,h as ra,q as la,w as ca,l as ya,o as $a,m as Sa,f as da}from"./init-Cncy7-aQ.js";/* empty css              */import{f as wa}from"./foo-BC8ccgDl.js";import{g as C,s as x,d as ua,C as X,N as v,m as Ta,e as Pa,h as ea,M as pa,r as Q,i as oa}from"./widev-B6MN93WZ.js";function F(){const a=C(`empleadosPuntos_${g}`);if(a)return sa(a);if(!h.length)return t(".app").html('<div class="estado-vacio"><i class="fa-solid fa-spinner fa-spin"></i><p>Cargando empleados...</p></div>');x(`empleadosPuntos_${g}`,h,5),sa(h)}function sa(a){const e=a.map((o,s)=>{const c=s+1,l=c===1?"champion":c===2?"runner-up":"",r=c===1?"crown":c===2?"medal":"user";return`
  ...
  `:`<div style="color:#666;text-align:center;padding:20px;font-style:italic">
    ...
    `);const[n,u]=b.sort((d,y)=>y[1].puntos-d[1].puntos)[0],m={ganador:n,puntosGanados:u.puntos,totalVentas:u.ventas,mes:r,year:l,mesCompleto:ua(`${a}-01`),fechaRegistro:na()};await ra(R(P,"ganadores",s),m),x(e,m,1440),U(m)}catch(a){console.error("Error ganador:",a),t("#lastWinner").html(`
      <div class="winner-header"><i class="fas fa-trophy"></i><h3>Ganador del Mes Anterior</h3></div>
      <div class="error-winner"><i class="fas fa-exclamation-triangle"></i><span>Error al cargar</span></div>
    `)}}function Da(a){const[e,o]=a.split("-"),s=new Date(parseInt(e),parseInt(o)-2);return`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`}function U(a){const e=h.find(o=>o.usuario===a.ganador||o.nombre===a.ganador);t("#lastWinner").html(`
    <div class="winner-header"><i class="fas fa-trophy"></i><h3>Ganador del Mes Anterior</h3></div>
  ...
  `)}function xa(){return`<div class="competition-summary" id="competitionSummary">
  ...
  </div>`}function A(){const a=new Date,e=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,[o,s]=g.split("-").map(Number),c=`resumenMes_${g}`,l=C(c);if(l&&l.mes===g&&l.dia===e){t("#toursHoy").text(l.toursHoy),t("#totalTours").text(l.totalTours),t("#totalPuntos").text(l.totalPuntos),t("#metaMes").text(l.meta);return}let r=0,i=0,p=0;w.forEach(n=>{const u=n.fechaTour;if(!u)return;let m,d,y;if(typeof u=="string"){const[T,I,ta]=u.split("-");m=+T,d=+I,y=u}else if(u.toDate){const T=u.toDate();m=T.getFullYear(),d=T.getMonth()+1,y=`${m}-${String(d).padStart(2,"0")}-${String(T.getDate()).padStart(2,"0")}`}else return;m===o&&d===s&&(r+=n.qventa||0,i+=n.puntos||0,y===e&&(p+=n.qventa||0))});const f=2500;t("#toursHoy").text(p),t("#totalTours").text(r),t("#totalPuntos").text(i),t("#metaMes").text(f),x(c,{mes:g,dia:e,toursHoy:p,totalTours:r,totalPuntos:i,meta:f},.25);const b=[Math.min(p/5*100,100),Math.min(r/50*100,100),Math.min(i/f*100,100),100];t(".summary-stat").each((n,u)=>{const m=b[n]/100*360;t(u).css({"--progress":`${m}deg`,"--width":`${b[n]}%`})})}function Ca(){return`
  ...
  `}let $=[],S=null;function Ea(){try{localStorage.removeItem("topSmiles"),localStorage.removeItem(`empleadosPuntos_${g}`),localStorage.removeItem(`resumenMes_${g}`)}catch(a){console.warn("Cache clear warn",a)}}t(document).off("click.btnsave").on("click.btnsave",".btn-save",async a=>{a.preventDefault();const e=t(".btn-save");if(e.prop("disabled"))return;if(!S)return v("Selecciona un tour","error"),t("#tourDisplay").addClass("faltaValor").focus();const s=[["#nombreCliente","Cliente"],["#horaSalida","Hora"],["#fechaTour","Fecha"],["#Operador","Operador"],["#metodoPago","Pago"]].filter(([f])=>!t(f).val()?.trim());if(s.length)return s.forEach(([f])=>t(f).addClass("faltaValor")),v("Completa: "+s.map(([,f])=>f).join(", "),"error");const c=parseInt(t("#cantidadPax").val())||1,l=t("#vtJulio,#vtSonia,#vtExterna").is(":checked"),r=e.attr("data-edit-id")||`venta_${Date.now()}`,i={idVenta:r,tipoTour:S.tour,registroEn:t("#registroEn").val(),nombreCliente:t("#nombreCliente").val(),numeroHabitacion:t("#numeroHabitacion").val(),tipoDocumento:t("#tipoDocumento").val(),numeroDocumento:t("#numeroDocumento").val(),cantidadPax:c,precioUnitario:parseFloat(t("#precioUnitario").val())||0,metodoPago:t("#metodoPago").val(),importeTotal:parseFloat(t("#importeTotal").val())||0,ganancia:parseFloat(t("#ganancia").val())||0,horaSalida:t("#horaSalida").val(),Operador:t("#Operador").val(),PagoOperador:parseFloat(t("#PagoOperador").val())||0,Comentario:t("#Comentario").val(),fechaTour:ua(t("#fechaTour").val()),estadoPago:t("#estadoPago").val(),vendedor:N.displayName,puntos:l?0:S.pts*c,email:N.email,qventa:1,fechaRegistro:na(),esVentaJulio:!!t("#vtJulio").prop("checked"),esVentaSonia:!!t("#vtSonia").prop("checked"),esVentaExterna:!!t("#vtExterna").prop("checked")},p='<i class="fas fa-save"></i> Guardar Venta';e.prop("disabled",!0).html('<i class="fas fa-spinner fa-spin"></i> Guardando...');try{await ra(R(P,"registrosdb",r),i),i.id=r;const f=w.findIndex(b=>b.id===r||b.idVenta===r);f>-1?w[f]=i:w.push(i),w.sort((b,n)=>{const u=b.fechaTour?.toDate?b.fechaTour.toDate():new Date(b.fechaTour||0);return(n.fechaTour?.toDate?n.fechaTour.toDate():new Date(n.fechaTour||0))-u}),window.currentPage!==void 0&&(window.currentPage=1),D(),Ea(),v(f>-1?"Venta actualizada":"Venta registrada","success"),B(),setTimeout(async()=>{await O(),F(),A()},40)}catch(f){console.error("Err venta:",f),v("Error guardando","error")}finally{e.prop("disabled",!1).html(p).removeAttr("data-edit-id")}});t(document).on("input change","#formularioVenta input, #formularioVenta select",function(){t(this).toggleClass("okValor faltaValor",!!t(this).val()?.toString().trim())});t(document).on("click",".tour-row",()=>t("#tourDisplay").removeClass("faltaValor").addClass("okValor"));t(document).on("change","#vtJulio, #vtSonia, #vtExterna",Y);t(document).on("input","#cantidadPax, #precioUnitario",()=>{ma(),_(),Y()});t(document).on("change","#estadoPago",_);t(document).on("input","#importeTotal, #PagoOperador",_);t(document).on("click",".btn-clear-view, .btn-cancel-edit",()=>{B(),v("Vista limpiada","info")});function ma(){const a=parseInt(t("#cantidadPax").val())||1,e=parseFloat(t("#precioUnitario").val())||0;t("#importeTotal").val((e*a).toFixed(2)),K("#importeTotal")}function _(){const a=t("#estadoPago").val(),e=parseFloat(t("#importeTotal").val())||0,o=parseFloat(t("#PagoOperador").val())||0,s=a==="pagado"||a==="pagado2";t("#ganancia").val((s?e:e-o).toFixed(2)),t("#PagoOperador").prop("disabled",s).attr("placeholder",s?"Servicio nuestro":"0.00").val(s?"0":t("#PagoOperador").val()),K("#ganancia")}function Y(){const a=parseInt(t("#cantidadPax").val())||1,e=S?.pts||0,o=t("#vtJulio, #vtSonia, #vtExterna").is(":checked");t("#vistaPreviaLaPuntos").text(o?0:e*a)}function K(a){t(a).addClass("field-updated"),setTimeout(()=>t(a).removeClass("field-updated"),1e3)}async function va(){try{console.log("🔄 Cargando tours...");const a=C("toursSmile");if(a?.length>0){$=a.map(s=>({nt:s.num||Math.random(),tour:s.tour,price:s.precio,pts:s.puntos,com:s.comision||5})),console.log(`✅ ${$.length} tours desde cache`),typeof M=="function"&&M();return}const e=await V(la(k(P,"listatours"),ca("activo","==",!0)));if(e.empty)return console.log("❌ No hay tours"),$=[];const o=e.docs.map(s=>({id:s.id,...s.data()}));$=o.map(s=>({nt:s.num||Math.random(),tour:s.tour,price:s.precio,pts:s.puntos,com:s.comision||5})),x("toursSmile",o,300),console.log(`✅ ${$.length} tours desde Firebase`),typeof M=="function"&&M()}catch(a){console.error("❌ Error tours:",a),v("Error cargando tours","error")}}function M(){W($),t("#tourDisplay").off("click").on("click",function(e){e.stopPropagation();const o=t("#tourDropdown");o.toggleClass("active"),t(this).toggleClass("active"),o.hasClass("active")&&setTimeout(()=>t("#tourSearch").focus(),50)});let a;t("#tourSearch").off("input").on("input",function(){const e=t(this).val().toLowerCase();clearTimeout(a),a=setTimeout(()=>{if(!e)return W($);e.length>=2&&W($.filter(o=>o.tour.toLowerCase().includes(e)||o.price.toString().includes(e)))},200)}),t(document).off("click.tour").on("click.tour",".tour-row",function(e){e.stopPropagation(),S=$[t(this).data("index")],S&&(t("#tourDisplay .tour-text").text(S.tour),t("#tipoTour").val(S.tour),t("#precioUnitario").val(S.price),t("#tourDropdown, #tourDisplay").removeClass("active"),t(".tour-row").removeClass("selected"),t(this).addClass("selected"),setTimeout(()=>{K("#precioUnitario"),ma(),_(),Y()},50))}),t(document).on("click",e=>{t(e.target).closest(".tour-selector").length||t("#tourDropdown, #tourDisplay").removeClass("active")}),Va(),ka()}function W(a){if(!a.length)return t("#tourTableBody").html('<tr><td colspan="4" style="text-align:center;color:#666;">Sin tours</td></tr>');const e=document.createDocumentFragment();a.forEach((s,c)=>{const l=document.createElement("tr");l.className="tour-row",l.dataset.index=$.indexOf(s),l.innerHTML=`<td class="tour-num">${c+1}</td><td class="tour-name">${s.tour}</td><td class="tour-price">S/ ${s.price}</td><td class="tour-pts">${s.pts} pts</td>`,e.appendChild(l)});const o=document.getElementById("tourTableBody");o.innerHTML="",o.appendChild(e)}function Va(){if(!$.length)return t("#pointsGrid").html('<p style="text-align:center;color:#666;">Sin datos</p>');const a=[...$].sort((e,o)=>o.pts-e.pts);t("#pointsGrid").html(a.map(e=>`<div class="point-item"><span class="service-name">${e.tour}</span><span class="point-value">${e.pts}</span></div>`).join(""))}function ka(){if(!$.length)return t("#pricesGrid").html('<p style="text-align:center;color:#666;">Sin datos</p>');t("#pricesGrid").html($.map(a=>`<div class="price-item"><span class="service-name">${a.tour}</span><span class="service-price">S/ ${a.price.toFixed(2)}</span></div>`).join(""))}function B(){S=null,t("#formularioVenta input, #formularioVenta select").prop("disabled",!1),t("#formularioVenta").removeClass("view-only edit-mode"),t(".btn-save").prop("disabled",!1).html('<i class="fas fa-save"></i> Guardar Venta').removeAttr("data-edit-id"),t(".btn-clear-view, .btn-cancel-edit").remove(),t("#formularioVenta")[0].reset(),t("#cantidadPax").val(1),t("#fechaTour").val(new Date().toISOString().split("T")[0]),t("#vistaPreviaLaPuntos").text("0"),t("#tourDisplay .tour-text").text("🔍 Seleccionar tour..."),t(".tour-row").removeClass("selected"),t("#importeTotal, #ganancia").prop("disabled",!0)}function ga(a,e=!1){B(),S=$.find(o=>o.tour===a.tipoTour||a.tipoTour.includes(o.tour.split(" ")[1])),S?(t("#tourDisplay .tour-text").text(S.tour),t("#tipoTour").val(S.tour),t(`.tour-row[data-tour*='"nt":${S.nt}']`).addClass("selected")):(t("#tourDisplay .tour-text").text(a.tipoTour||"🔍 Seleccionar..."),t("#tipoTour").val(a.tipoTour||"")),Object.entries(a).forEach(([o,s])=>{const c=t(`#${o}`);c.length&&(o==="fechaTour"&&s?.toDate?c.val(s.toDate().toISOString().split("T")[0]):c.val(s||""))}),t("#vtJulio").prop("checked",a.esVentaJulio||!1),t("#vtSonia").prop("checked",a.esVentaSonia||!1),t("#vtExterna").prop("checked",a.esVentaExterna||!1),_(),Y(),e?(t("#formularioVenta input, #formularioVenta select, .tour-display").prop("disabled",!0),t(".btn-save").prop("disabled",!0).html('<i class="fas fa-eye"></i> Solo Vista'),t("#formularioVenta").addClass("view-only"),t(".btn-clear-view").length||t(".form-actions").prepend('<button type="button" class="btn-clear-view" style="background:#6c757d;color:#fff;border:none;padding:10px 20px;border-radius:5px;margin-right:10px"><i class="fas fa-times"></i> Limpiar Vista</button>')):(t("#formularioVenta input, #formularioVenta select").prop("disabled",!1),t(".tour-display").prop("disabled",!1),t("#formularioVenta").addClass("edit-mode"),t(".btn-cancel-edit").length||t(".form-actions").prepend('<button type="button" class="btn-cancel-edit" style="background:#dc3545;color:#fff;border:none;padding:10px 20px;border-radius:5px;margin-right:10px"><i class="fas fa-times"></i> Cancelar Edición</button>'))}async function ha(){try{const a=C("empleadosSmile");a&&(h.splice(0,h.length,...a),F());const o=(await V(k(P,"smiles"))).docs.filter(s=>s.data().participa==="si").map(s=>({id:s.id,...s.data()}));h.splice(0,h.length,...o),x("empleadosSmile",h,300),await O(),F()}catch(a){console.error("Error empleados:",a),t("#workersGrid").html('<div class="error-workers"><i class="fas fa-exclamation-triangle"></i>Error</div>')}}t(document).on("change","#mostrarn",function(){La(parseInt(t(this).val())),j(1),D(t("#filterEmployee").val())});t(document).on("change","#monthSelector",function(){Ra(t(this).val()),j(1),Promise.all([O(),aa()]).then(()=>{F(),D(),A(),Z()})});t(document).on("change","#filterEmployee",()=>{j(1),D(t("#filterEmployee").val())});t(document).on("click","#todayFilter",()=>{j(1),D(t("#filterEmployee").val(),!0)});window.cambiarPagina=a=>{j(a),D(t("#filterEmployee").val())};window.verDetalleVenta=a=>{const e=w.find(o=>o.id===a);if(!e)return v("Venta no encontrada","error");ga(e,!0),document.querySelector("#formularioVenta").scrollIntoView({behavior:"smooth",block:"start"}),v("Datos cargados","info")};window.editarVenta=a=>{const e=w.find(o=>o.id===a);if(!e)return v("Venta no encontrada","error");ga(e,!1),t(".btn-save").html('<i class="fas fa-edit"></i> Actualizar Venta').attr("data-edit-id",a),document.querySelector("#formularioVenta").scrollIntoView({behavior:"smooth",block:"start"}),v("Datos cargados para edición","info")};window.eliminarVenta=a=>{const e=w.find(o=>o.id===a);if(!e)return v("Venta no encontrada","error");confirm(`¿Eliminar venta de "${e.nombreCliente}"?
...
¿CONFIRMAS?`)&&Ma(a)};async function Ma(a){try{v("Eliminando...","info");const e=w.find(i=>i.id===a),o=(()=>{const i=e?.fechaTour;if(!i)return g;if(typeof i=="string"){const[f,b]=i.split("-");return`${f}-${b}`}const p=i.toDate();return`${p.getFullYear()}-${String(p.getMonth()+1).padStart(2,"0")}`})(),[s,c]=g.split("-").map(Number),l=(()=>{const i=new Date(s,c-2,1);return`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}`})();await ya(R(P,"registrosdb",a)),localStorage.removeItem(`empleadosPuntos_${o}`),localStorage.removeItem(`resumenMes_${o}`),o===l&&localStorage.removeItem(`ganadorMes_${l}`),Object.keys(localStorage).filter(i=>i.startsWith("vendedor_")).forEach(i=>{try{JSON.parse(localStorage.getItem(i))?.idVenta===a&&localStorage.removeItem(i)}catch{}});const r=w.findIndex(i=>i.id===a);r>-1&&w.splice(r,1),o===g&&(await O(),A(),F()),D(),o===l&&await Z(),t(".btn-save").attr("data-edit-id")===a&&B(),v("¡Venta eliminada!","success")}catch(e){console.error("Error eliminando:",e),v("Error al eliminar","error")}}function Na(){const a=h.map(e=>`<option value="${e.usuario}">${e.nombre}</option>`).join("");t("#filterEmployee").html(`<option value="">Todos los vendedores</option>${a}`)}async function aa(){try{const a=await V(k(P,"registrosdb"));w.splice(0,w.length,...a.docs.map(e=>({id:e.id,...e.data()}))),w.sort((e,o)=>{const s=e.fechaTour?.toDate?e.fechaTour.toDate():new Date(e.fechaTour||"1970");return(o.fechaTour?.toDate?o.fechaTour.toDate():new Date(o.fechaTour||"1970"))-s}),D()}catch(a){console.error("Error ventas:",a),t("#salesTableBody").html('<tr><td colspan="11" class="error-cell"><i class="fas fa-exclamation-triangle"></i>Error</td></tr>')}}function D(a="",e=!1){const[o,s]=g.split("-").map(Number),c=new Date,l=`${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,"0")}-${String(c.getDate()).padStart(2,"0")}`;let r=w.filter(n=>{const u=n.fechaTour;if(!u)return!1;if(u.toDate){const m=u.toDate();return m.getFullYear()===o&&m.getMonth()+1===s}if(typeof u=="string"){const[m,d]=u.split("-").map(Number);return m===o&&d===s}return!1});a&&(r=r.filter(n=>n.vendedor===a)),e&&(r=r.filter(n=>{const u=n.fechaTour;if(!u)return!1;if(typeof u=="string")return u===l;if(u.toDate){const m=u.toDate();return`${m.getFullYear()}-${String(m.getMonth()+1).padStart(2,"0")}-${String(m.getDate()).padStart(2,"0")}`===l}return!1}));const i=Math.ceil(r.length/L),p=(E-1)*L,b=r.slice(p,p+L).map(n=>{const m=n.vendedor===N?.displayName?`<button class="btn-view" onclick="verDetalleVenta('${n.id}')"><i class="fas fa-eye"></i></button>
      ...
      <tr>
        ...
        <td><span class="status-badge ${I.cls}">
          <i class="fas fa-${I.icn}"></i> ${I.txt}
        </span></td>
  ...
  `}const q={empleados:300,tours:300,ventas:180,notas:600},J=a=>localStorage.setItem(`${a}_timestamp`,Date.now()),H=(a,e)=>{const o=localStorage.getItem(`${a}_timestamp`);return!o||(Date.now()-+o)/1e3>e},z=(a,e)=>{const o=a.find("i");e?(o.attr("class","fas fa-spinner fa-spin"),a.prop("disabled",!0)):(o.attr("class","fa-solid fa-rotate-right"),a.prop("disabled",!1))};function _a(){return{empleados:H("empleadosSmile",q.empleados),tours:H("toursSmile",q.tours),ventas:H("ventasSmile",q.ventas),notas:H("notasSmile",q.notas)}}async function ja(){Q("empleadosSmile"),await ha(),J("empleadosSmile")}async function Ga(){Q("toursSmile"),await va(),J("toursSmile")}async function qa(){await aa(),J("ventasSmile")}async function Ha(){Q("notasSmile"),await fa(),J("notasSmile")}async function za(){const a=t(".wifresh");z(a,!0),v("🔄 Revisando cambios","info");const e=_a(),o=[];if(e.empleados&&o.push(ja()),e.tours&&o.push(Ga()),e.ventas&&o.push(qa()),e.notas&&o.push(Ha()),!o.length)return z(a,!1),v("✅ Todo al día","success");v(`🔄 Aplicando ${o.length} actualizaciones`,"info");try{await Promise.all(o),(e.ventas||e.empleados)&&(await O(),A()),e.empleados&&F(),e.ventas&&D(),e.tours&&M(),z(a,!1),v("✅ Datos actualizados","success")}catch(s){console.error("wifresh error",s),z(a,!1),v("❌ Falló actualización","error")}}t(document).on("click",".bt_cargar",()=>{["empleadosSmile","toursSmile","ventasSmile","notasSmile","empleadosPuntos_","topSmiles"].forEach(a=>{Object.keys(localStorage).forEach(e=>e.startsWith(a)&&localStorage.removeItem(e))}),pa("✅ Cache limpiado"),setTimeout(()=>location.reload(),600)});t(document).on("click",".wifresh",a=>{a.preventDefault(),za()});let N=null,g="2025-09",E=1,L=5,w=[],h=[];const La=a=>{L=a},j=a=>{E=a},Ra=a=>{g=a};$a(da,async a=>{if(!a)return window.location.href="/";N=a;try{const e=C("wiSmile");if(e)return ia(e),oa(P,N);const s=(await V(la(k(P,"smiles"),ca("usuario","==",a.displayName)))).docs[0].data();x("wiSmile",s,450),ia(s),oa(P,N)}catch(e){console.error(e)}});t(document).on("click",".bt_salir",async()=>{await Sa(da),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(e=>localStorage.removeItem(e))}});async function ia(a){pa(`Bienvenido ${a.nombre}!`),t(".app").html(`
    <header class="top-header">
      <div class="header-container miwp">
        <div class="header-left">
          <h1 class="main-title"><i class="fas fa-trophy"></i>RETO DEL MES</h1>
          <select id="monthSelector" class="month-selector">${Ya()}</select>
        </div>
    ...
    ${wa()}
  `),t("#monthSelector").val(g);try{await Promise.all([ha(),aa(),Z(),va(),fa()]),Na(),M(),A()}catch(e){console.error("ErrorIn:",e)}}function Ya(){const a=new Date,e=a.getFullYear(),o=a.getMonth(),s=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];return g=`${e}-${String(o+1).padStart(2,"0")}`,t.map(new Array(7),(c,l)=>{const r=l-3,i=o+r,p=e+Math.floor(i/12),f=(i%12+12)%12;return`<option value="${`${p}-${String(f+1).padStart(2,"0")}`}"${r===0?" selected":""}>${s[f]} ${p}</option>`}).join("")}
